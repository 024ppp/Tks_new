...
*  create        : 1999.10                                             *
*  update        :                                                     *
*  contents      : ブラシ幅寸法（縦押し）計測 メインプログラム         *
...
//---------------------------------------------------------------------------
// KD21構造体変数
structKD21 KD21Data[30];      //ブラシ幅寸法（縦押し）ﾃﾞｰﾀﾌｧｲﾙ  KD21

//KD23W構造体変数
structKD23W KD23WData[30];    //ブラシ幅寸法（縦押し）計測履歴ﾃﾞｰﾀﾜｰｸﾌｧｲﾙ  KD23W


//---------------------------------------------------------------------------
//画面操作用変数
int iRec_Kd21;              // 一覧検索結果件数
...
long  Kd23W_Create01(AnsiString, int, int) ;
int  Kd23W_Serch01();
...
void ExecF1(void)
{
   if( Form1->edtymd->Focused() || Form1->edttlno->Focused() ){
      ...
      //ブラシ幅寸法（縦押し）計測ﾃﾞｰﾀﾌｧｲﾙからデータを抽出
     iRec_Kd21 = Kd21_Serch01() ;
...
void __fastcall TForm1::BtnF07Click(TObject *Sender)
{
   ...
   iKskStart = 0 ;
    ...
    // ブラシ幅寸法ﾃﾞｰﾀﾌｧｲﾙからﾃﾞｰﾀを抽出  KD21
    int iRtn = Kd21_Serch02();
   if( iRtn <= 0 ) {
   ...
   //画面の初期化
    ...
    // 計測値の変更があったかどうかの指示フラグ
     ...
     //// KD23W ブラシ幅寸法（縦押し）計測履歴ﾃﾞｰﾀテーブルの作成
     // Kd23W_Create01(KD21Data[0].KENSA_YMD, KD21Data[0].TOLEY_NO, LOTMAX);
     // //if( iRec >0 ){
     //
     //   //KD23Wブラシ幅寸法（縦押し）計測履歴ﾃﾞｰﾀﾌｧｲﾙを読み込む
     //   i  =  Kd23W_Serch01();
     //// }
	 ...
	 if( CheckBoxRireki->Checked ){
	     ...
	     //KD23W履歴ﾃﾞｰﾀテーブルの作成
         //2002/11/25 E.Takase
	     //int iRec = Kd23W_Create01(KD21Data[0].KENSA_YMD, KD21Data[0].TOLEY_NO, LOTMAX);
	     int iRec = Kd23W_Create01(strVal01, strVal02.ToIntDef(0), LOTMAX);

	     if( iRec >0 ){

	        //KD23W履歴ﾃﾞｰﾀﾌｧｲﾙを読み込む
	        i  =  Kd23W_Serch01();
...
//        ToDrawGraph(1, iOption);
     ...
     if( HaveSokutei ) CopyKD21Data();

     // 指定した計測予定日とﾄﾚｰNOで検索した結果を画面へセット
    int flag = 0;
    for( i=0; i<30; i++ )
    {
          //ｻﾝﾌﾟﾙﾎﾟｼﾞｼｮﾝNOがないの場合
          if( KD21Data[i].SAMPLE_P == -1 )
          {
               // ﾃﾞｰﾀ１、ﾃﾞｰﾀ２ｺﾝﾄﾛｰﾙ
              /* IsEnabledEdt( pEdt1[i], 0 );
                ...
                */
                ...
                //ﾃﾞｰﾀ１、ﾃﾞｰﾀ２の設定
               pEdt1[i]->Text =  FormatFloat("#0.000",KD21Data[i].SOKUTEICHI[0] );
               pEdt2[i]->Text =  FormatFloat("#0.000",KD21Data[i].SOKUTEICHI[1] );
               pPnlN[i]->Caption = IntToStr( KD21Data[i].SOKUTEI_NM) ;

               // 例外ﾃﾞｰﾀ の設定
               if( KD21Data[i].SOKUTEI_NM > 2 )
               {
                 pEdt3[i]->Text =  FormatFloat("#0.000",KD21Data[i].SOKUTEICHI[2] );
                ...
                if(  KD21Data[i].SOKUTE_KBN == 1 ){
                  ...
                  if( KD21Data[i].SOKUTEI_NM > 2 )
                  {
                    ...
                    // ScrollBarの初期値設定
                    pScrl[i]->Min = 3;
                    pScrl[i]->Max = KD21Data[i].SOKUTEI_NM;
                    pScrl[i]->Position = 3 ;
                  ...
                  iLastPosition = KD21Data[i].SAMPLE_P ;
       ...
       ( SamplePositionIdx >= iFirstPosition && SamplePositionIdx <= iLastPosition ) &&
         DataNoIdx > 2   ){
          ...
          pEdt3[SamplePositionIdx-1]->Text = FormatFloat("#0.000",
                                      KD21Data[SamplePositionIdx-1].SOKUTEICHI[pos-1] );
...
void __fastcall TForm1::Edt01_1Change(TObject *Sender)
{
   ...
   //  TNumberEdit *Edt = (TNumberEdit *)Sender;
     ...
     CurDataChg = 1;
        ...
        // 計測したデータCount
     if ( KD21Data[SamplePositionIdx-1].SOKUTEI_NM_CUR < DataNoIdx ){
             KD21Data[SamplePositionIdx-1].SOKUTEI_NM_CUR = DataNoIdx ;
     }
...
void __fastcall TForm1::Edt01_1Exit(TObject *Sender)
{
   ...
   if( iKskStart && ! CurBackFlg && SamplePositionIdx >= 1) {
      ...
      if( CurDataChg) {
       ...
       //ｻﾝﾌﾟﾙﾎﾟｼﾞｼｮﾝ情報の表示、履歴作画と工程能力の表示
       CheckInKikaku(1);
...
void __fastcall TForm1::Edt01_4Change(TObject *Sender)
{
  ...
  if( iKskStart ) {
     ...
     if( n >= 3 && n <= KD21Data[SamplePositionIdx-1].SOKUTEI_NM ){
         ...
         pEdt3[SamplePositionIdx-1]->Text = FormatFloat("#0.000",KD21Data[SamplePositionIdx-1].SOKUTEICHI[n-1] );
...
void __fastcall TForm1::SetMEMO(void)
{
	...
	for( i=0; i<30; i++ )
    {
		...
		if( KD21Data[i].SAMPLE_P == -1 )
		{
			...
			if ( Trim(KD21Data[i].MEMO) != "" ) {
				//pPnlP[i]->Hint = KD21Data[i].MEMO;
				pEdt1[i]->Hint = KD21Data[i].MEMO;
				pEdt2[i]->Hint = KD21Data[i].MEMO;
				pEdt3[i]->Hint = KD21Data[i].MEMO;
				pEdt4[i]->Hint = KD21Data[i].MEMO;
				pScrl[i]->Hint = KD21Data[i].MEMO;
				pPnlN[i]->Hint = KD21Data[i].MEMO;
