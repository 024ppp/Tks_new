...
*  create        : 1999.10                                             *
*  update        :                                                     *
*  contents      : ブラシ幅寸法（縦押し）計測測定履歴、工程能力作画関数*
...
void DispKoutei( int iSp )
{
   ...
   //表示データを求める
   int iLinkSp;
   if(  KD21Data[iSp-1].LINK == 0 ){
       iLinkSp = iSp ;
   }else {
       iLinkSp = KD21Data[iSp-1].LINK;
   }
   ...
   for( i=iLotStart; i < iRtn ; i++ ) {
          ...
          if( KD23WData[iLinkSp-1].SOKUTEI_NM[i] == 0 ) continue ;

           //履歴データ個数
           n += KD23WData[iLinkSp-1].SOKUTEI_NM[i] ;

           // 平均値の計算
           dAve += KD23WData[iLinkSp-1].SOKUTE_AVE[i]*KD23WData[iLinkSp-1].SOKUTEI_NM[i];

           for( j=0; j< KD23WData[iLinkSp-1].SOKUTEI_NM[i]; j++ ) {

                if( KD23WData[iLinkSp-1].SOKUTEI[i][j] > dMax ) dMax = KD23WData[iLinkSp-1].SOKUTEI[i][j];
                if( KD23WData[iLinkSp-1].SOKUTEI[i][j] < dMin ) dMin = KD23WData[iLinkSp-1].SOKUTEI[i][j];

                //不合格ﾃﾞｰﾀのﾁｪｯｸ
                if(  KD23WData[iLinkSp-1].SOKUTEI[i][j] >  KD23WData[iLinkSp-1].BURASIT_MAX[i] ||
                     KD23WData[iLinkSp-1].SOKUTEI[i][j] <  KD23WData[iLinkSp-1].BURASIT_MIN[i] )
                 FuryoCnt +=1 ;
   ...
   //ｶﾚﾝﾄｶｰｿﾙがある計測ﾃﾞｰﾀ
   if( KD21Data[iSp-1].SOKUTEI_NM_CUR != 0 )
   {
            //データ個数
            n += KD21Data[iSp-1].SOKUTEI_NM_CUR ;

            // 平均値の計算
            for( j=0; j< KD21Data[iSp-1].SOKUTEI_NM_CUR; j++ ) {
                dAve += KD21Data[iSp-1].SOKUTEICHI[j];

               if( KD21Data[iSp-1].SOKUTEICHI[j] > dMax ) dMax = KD21Data[iSp-1].SOKUTEICHI[j];
               if( KD21Data[iSp-1].SOKUTEICHI[j] < dMin ) dMin = KD21Data[iSp-1].SOKUTEICHI[j];

               //不合格ﾃﾞｰﾀのﾁｪｯｸ
                if(  KD21Data[iSp-1].SOKUTEICHI[j] >  KD21Data[iSp-1].BURASIT_MAX ||
                     KD21Data[iSp-1].SOKUTEICHI[j] <  KD21Data[iSp-1].BURASIT_MIN )
                 FuryoCnt +=1 ;
   ...
   if( n!= 1 ){
       ...
       for( i=iLotStart; i<iRtn ; i++ )
       {
           for( int j=0; j< KD23WData[iLinkSp-1].SOKUTEI_NM[i]; j++ )
                dSigma += (KD23WData[iLinkSp-1].SOKUTEI[i][j] - dAve)*(KD23WData[iLinkSp-1].SOKUTEI[i][j] - dAve) ;
       }

       if( KD21Data[iSp-1].SOKUTEI_NM_CUR != 0 ) {
           for( i=0; i<KD21Data[iSp-1].SOKUTEI_NM_CUR ; i++ )
                dSigma += (KD21Data[iSp-1].SOKUTEICHI[i] - dAve)*(KD21Data[iSp-1].SOKUTEICHI[i] - dAve) ;
       }
   ...
   double dA = KD21Data[iSp-1].BURASIT_MAX + KD21Data[iSp-1].BURASIT_MIN ;
   double dB = KD21Data[iSp-1].BURASIT_MAX - KD21Data[iSp-1].BURASIT_MIN ;
...
void SetRirekiData(int iSp)
{
  ...
  // 作画用変数の初期化
   ...
   for( i=iLotStart; i < iRtn ; i++ ) {
          ...
          if( KD23WData[iLinkSp-1].SOKUTEI_NM[i] != 0 ) {

              // 最大値、最小値、平均値の計算
              Max[i-iLotStart] = maxa;
              Min[i-iLotStart] = mina ;
              for(  j=0; j<KD23WData[iLinkSp-1].SOKUTEI_NM[i] ; j++ ){
                    if( KD23WData[iLinkSp-1].SOKUTEI[i][j] > Max[i-iLotStart] ) Max[i-iLotStart] = KD23WData[iLinkSp-1].SOKUTEI[i][j];
                    if( KD23WData[iLinkSp-1].SOKUTEI[i][j] < Min[i-iLotStart] ) Min[i-iLotStart] = KD23WData[iLinkSp-1].SOKUTEI[i][j];

                    Val[i-iLotStart] += KD23WData[iLinkSp-1].SOKUTEI[i][j] ;
              }

              Val[i-iLotStart] = Val[i-iLotStart]/KD23WData[iLinkSp-1].SOKUTEI_NM[i] ;
          }

          str = KD23WData[iLinkSp-1].LOTNO[i] ;
   ...
   if( KD21Data[iSp-1].SOKUTEI_NM_CUR != 0 )
   {
         ...
         Min[iRRR] = mina ;
         for( i=0; i< KD21Data[iSp-1].SOKUTEI_NM_CUR ; i++ ){
              if( KD21Data[iSp-1].SOKUTEICHI[i] > Max[iRRR] ) Max[iRRR] = KD21Data[iSp-1].SOKUTEICHI[i];
              if( KD21Data[iSp-1].SOKUTEICHI[i] < Min[iRRR] ) Min[iRRR] = KD21Data[iSp-1].SOKUTEICHI[i];
         }

         // 平均値の計算
         for( i=0; i< KD21Data[iSp-1].SOKUTEI_NM_CUR; i++ )
              Val[iRRR] += KD21Data[iSp-1].SOKUTEICHI[i] ;
         Val[iRRR] = Val[iRRR]/KD21Data[iSp-1].SOKUTEI_NM_CUR ;
   }

   str = KD21Data[iSp-1].LOTNO ;
   if( str.Length() > 5 ) str = str.Delete(1,1);
...
void DrawRireki(TImage *GraphName,int n, char *Pstr, double Max[10], double Min[10], double Val[10], int iCur)
{
    ...
    //最大値、最小値の検索
    MaxY = KD21Data[SamplePositionIdx-1].BURASIT_MAX ;
    MinY = KD21Data[SamplePositionIdx-1].BURASIT_MIN;
...
{
   //画面へデータ設定
   Form1->Pnl24->Caption = FormatFloat("#0.00", iN );
...
*  create        : 1999.10                                             *
*  update        :                                                     *
*  contents      : KD23Wから読み込んだﾃﾞｰﾀ中にに指定したロットNo       *
*                :  有るかどうか のチェック                            *
*  written by    : 呉 亜娟(NKK unics Corp.)                            *
***********************************************************************/
int  CheckLotNo(int iSp)
{
   ...
   for( int i=0; i<  KD23WData[iLinkSp-1].LOTCNT; i++ ){
      if(  KD21Data[iSp-1].LOTNO ==  KD23WData[iLinkSp-1].LOTNO[i] )
      {
        iFlag = i ;
        break ;
       }
