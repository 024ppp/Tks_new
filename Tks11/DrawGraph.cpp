...
*  create        : 1999.10                                             *
*  update        :                                                     *
*  contents      : ブラシ幅寸法計測測定履歴、工程能力作画関数          *
...
void DispKoutei( int iSp )
{
   ...
   //表示データを求める
   int iLinkSp;
   if(  KD11Data[iSp-1].LINK == 0 ){
       iLinkSp = iSp ;
   }else {
       iLinkSp = KD11Data[iSp-1].LINK;
   }
   ...
   for( i=iLotStart; i < iRtn ; i++ ) {
          ...
          if( KD13WData[iLinkSp-1].SOKUTEI_NM[i] == 0 ) continue ;

           //履歴データ個数
           n += KD13WData[iLinkSp-1].SOKUTEI_NM[i] ;

           // 平均値の計算
           dAve += KD13WData[iLinkSp-1].SOKUTE_AVE[i]*KD13WData[iLinkSp-1].SOKUTEI_NM[i];

           for( j=0; j< KD13WData[iLinkSp-1].SOKUTEI_NM[i]; j++ ) {

                if( KD13WData[iLinkSp-1].SOKUTEI[i][j] > dMax ) dMax = KD13WData[iLinkSp-1].SOKUTEI[i][j];
                if( KD13WData[iLinkSp-1].SOKUTEI[i][j] < dMin ) dMin = KD13WData[iLinkSp-1].SOKUTEI[i][j];

                //不合格ﾃﾞｰﾀのﾁｪｯｸ
                if(  KD13WData[iLinkSp-1].SOKUTEI[i][j] >  KD13WData[iLinkSp-1].BURASI_MAX[i] ||
                     KD13WData[iLinkSp-1].SOKUTEI[i][j] <  KD13WData[iLinkSp-1].BURASI_MIN[i] )
                 FuryoCnt +=1 ;
   ...
   //ｶﾚﾝﾄｶｰｿﾙがある計測ﾃﾞｰﾀ
   if( KD11Data[iSp-1].SOKUTEI_NM_CUR != 0 )
   {
            //データ個数
            n += KD11Data[iSp-1].SOKUTEI_NM_CUR ;

            // 平均値の計算
            for( j=0; j< KD11Data[iSp-1].SOKUTEI_NM_CUR; j++ ) {
                dAve += KD11Data[iSp-1].SOKUTEICHI[j];

               if( KD11Data[iSp-1].SOKUTEICHI[j] > dMax ) dMax = KD11Data[iSp-1].SOKUTEICHI[j];
               if( KD11Data[iSp-1].SOKUTEICHI[j] < dMin ) dMin = KD11Data[iSp-1].SOKUTEICHI[j];

               //不合格ﾃﾞｰﾀのﾁｪｯｸ
                if(  KD11Data[iSp-1].SOKUTEICHI[j] >  KD11Data[iSp-1].BURASI_MAX ||
                     KD11Data[iSp-1].SOKUTEICHI[j] <  KD11Data[iSp-1].BURASI_MIN )
                 FuryoCnt +=1 ;
           }
   ...
   if( n!= 1 ){
       ...
       for( i=iLotStart; i<iRtn ; i++ )
       {
           for( int j=0; j< KD13WData[iLinkSp-1].SOKUTEI_NM[i]; j++ )
                dSigma += (KD13WData[iLinkSp-1].SOKUTEI[i][j] - dAve)*(KD13WData[iLinkSp-1].SOKUTEI[i][j] - dAve) ;
       }

       if( KD11Data[iSp-1].SOKUTEI_NM_CUR != 0 ) {
           for( i=0; i<KD11Data[iSp-1].SOKUTEI_NM_CUR ; i++ )
                dSigma += (KD11Data[iSp-1].SOKUTEICHI[i] - dAve)*(KD11Data[iSp-1].SOKUTEICHI[i] - dAve) ;
       }
   ...
   //K
   double dA = KD11Data[iSp-1].BURASI_MAX + KD11Data[iSp-1].BURASI_MIN ;
   double dB = KD11Data[iSp-1].BURASI_MAX - KD11Data[iSp-1].BURASI_MIN ;
...
void SetRirekiData(int iSp)
{
  ...
  // 作画用変数の初期化
   ...
   for( i=iLotStart; i < iRtn ; i++ ) {
          ...
          if( KD13WData[iLinkSp-1].SOKUTEI_NM[i] != 0 ) {

              // 最大値、最小値、平均値の計算
              Max[i-iLotStart] = maxa;
              Min[i-iLotStart] = mina ;
              for(  j=0; j<KD13WData[iLinkSp-1].SOKUTEI_NM[i] ; j++ ){
                    if( KD13WData[iLinkSp-1].SOKUTEI[i][j] > Max[i-iLotStart] ) Max[i-iLotStart] = KD13WData[iLinkSp-1].SOKUTEI[i][j];
                    if( KD13WData[iLinkSp-1].SOKUTEI[i][j] < Min[i-iLotStart] ) Min[i-iLotStart] = KD13WData[iLinkSp-1].SOKUTEI[i][j];

                    Val[i-iLotStart] += KD13WData[iLinkSp-1].SOKUTEI[i][j] ;
              }

              Val[i-iLotStart] = Val[i-iLotStart]/KD13WData[iLinkSp-1].SOKUTEI_NM[i] ;
          }

          str = KD13WData[iLinkSp-1].LOTNO[i] ;
   ...
   if( KD11Data[iSp-1].SOKUTEI_NM_CUR != 0 )
   {
         ...
         Min[iRRR] = mina ;
         for( i=0; i< KD11Data[iSp-1].SOKUTEI_NM_CUR ; i++ ){
              if( KD11Data[iSp-1].SOKUTEICHI[i] > Max[iRRR] ) Max[iRRR] = KD11Data[iSp-1].SOKUTEICHI[i];
              if( KD11Data[iSp-1].SOKUTEICHI[i] < Min[iRRR] ) Min[iRRR] = KD11Data[iSp-1].SOKUTEICHI[i];
         }

         // 平均値の計算
         for( i=0; i< KD11Data[iSp-1].SOKUTEI_NM_CUR; i++ )
              Val[iRRR] += KD11Data[iSp-1].SOKUTEICHI[i] ;
         Val[iRRR] = Val[iRRR]/KD11Data[iSp-1].SOKUTEI_NM_CUR ;
   }

   str = KD11Data[iSp-1].LOTNO ;
   if( str.Length() > 5 ) str = str.Delete(1,1);
...
void DrawRireki(TImage *GraphName,int n, char *Pstr, double Max[10], double Min[10], double Val[10], int iCur)
{
    ...
    //最大値、最小値の検索
    MaxY = KD11Data[SamplePositionIdx-1].BURASI_MAX ;
    MinY = KD11Data[SamplePositionIdx-1].BURASI_MIN;
...
*  create        : 1999.10                                             *
*  update        :                                                     *
*  contents      : KD13Wから読み込んだﾃﾞｰﾀ中にに指定したロットNo       *
*                :  有るかどうか のチェック                            *
*  written by    : 呉 亜娟(NKK unics Corp.)                            *
***********************************************************************/
int  CheckLotNo(int iSp)
{
   ...
   for( int i=0; i<  KD13WData[iLinkSp-1].LOTCNT; i++ ){
      if(  KD11Data[iSp-1].LOTNO ==  KD13WData[iLinkSp-1].LOTNO[i] )
      {
        iFlag = i ;
        break ;
       }
