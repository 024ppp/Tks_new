...
void DispKoutei( int iSp )
{
   ...
   //表示データを求める
   int iLinkSp;
   if(  KDC1Data[iSp-1].LINK == 0 ){
       iLinkSp = iSp ;
   }else {
       iLinkSp = KDC1Data[iSp-1].LINK;
   }
   ...
   for( i=iLotStart; i < iRtn ; i++ ){
          ...
          if(KDC3WData[iLinkSp-1].SOKUTEI_NM[i] == 0 ) continue ;

           //履歴データ個数
           n += KDC3WData[iLinkSp-1].SOKUTEI_NM[i] ;

           // 平均値の計算
           dAve += KDC3WData[iLinkSp-1].SOKUTE_AVE[i]*KDC3WData[iLinkSp-1].SOKUTEI_NM[i];

           for( j=0; j< KDC3WData[iLinkSp-1].SOKUTEI_NM[i]; j++ ) {

                if( KDC3WData[iLinkSp-1].SOKUTEI[i][j] > dMax ) dMax = KDC3WData[iLinkSp-1].SOKUTEI[i][j];
                if( KDC3WData[iLinkSp-1].SOKUTEI[i][j] < dMin ) dMin = KDC3WData[iLinkSp-1].SOKUTEI[i][j];

                //不合格ﾃﾞｰﾀのﾁｪｯｸ
                if(  KDC3WData[iLinkSp-1].SOKUTEI[i][j] >  KDC3WData[iLinkSp-1].MAGEG_MAX[i] ||
                     KDC3WData[iLinkSp-1].SOKUTEI[i][j] <  KDC3WData[iLinkSp-1].MAGEG_MIN[i] )
                 FuryoCnt +=1 ;
           }
   }

   //ｶﾚﾝﾄｶｰｿﾙがある計測ﾃﾞｰﾀ
   if( KDC1Data[iSp-1].SOKUTEI_NM_CUR != 0 )
   {

            //データ個数
            n += KDC1Data[iSp-1].SOKUTEI_NM_CUR ;

            // 平均値の計算
            for( j=0; j< KDC1Data[iSp-1].SOKUTEI_NM_CUR; j++ ) {
                dAve += KDC1Data[iSp-1].SOKUTEI_M[j];

               if( KDC1Data[iSp-1].SOKUTEI_M[j] > dMax ) dMax = KDC1Data[iSp-1].SOKUTEI_M[j];
               if( KDC1Data[iSp-1].SOKUTEI_M[j] < dMin ) dMin = KDC1Data[iSp-1].SOKUTEI_M[j];

               //不合格ﾃﾞｰﾀのﾁｪｯｸ
                if(  KDC1Data[iSp-1].SOKUTEI_M[j] >  KDC1Data[iSp-1].MAGEG_MAX ||
                     KDC1Data[iSp-1].SOKUTEI_M[j] <  KDC1Data[iSp-1].MAGEG_MIN )
                 FuryoCnt +=1 ;
           }
   ...
   if( n!= 1 ){
       ...
       for( i=iLotStart; i<iRtn ; i++ )
       {
           for( int j=0; j< KDC3WData[iLinkSp-1].SOKUTEI_NM[i]; j++ )
                dSigma += (KDC3WData[iLinkSp-1].SOKUTEI[i][j] - dAve)*(KDC3WData[iLinkSp-1].SOKUTEI[i][j] - dAve) ;
       }

       if( KDC1Data[iSp-1].SOKUTEI_NM_CUR != 0 ) {
           for( i=0; i<KDC1Data[iSp-1].SOKUTEI_NM_CUR ; i++ )
                dSigma += (KDC1Data[iSp-1].SOKUTEI_M[i] - dAve)*(KDC1Data[iSp-1].SOKUTEI_M[i] - dAve) ;
       }
   ...
   //K
   double dA = KDC1Data[iSp-1].MAGEG_MAX + KDC1Data[iSp-1].MAGEG_MIN ;
   double dB = KDC1Data[iSp-1].MAGEG_MAX - KDC1Data[iSp-1].MAGEG_MIN ;
...
void SetRirekiData(int iSp)
{
  ...
  // 作画用変数の初期化
   ...
   for( i=iLotStart; i < iRtn ; i++ ) {
          ...
          if( KDC3WData[iLinkSp-1].SOKUTEI_NM[i] != 0 ) {

              // 最大値、最小値、平均値の計算
              Max[i-iLotStart] = maxa;
              Min[i-iLotStart] = mina ;
              for(  j=0; j<KDC3WData[iLinkSp-1].SOKUTEI_NM[i] ; j++ ){
                    if( KDC3WData[iLinkSp-1].SOKUTEI[i][j] > Max[i-iLotStart] ) Max[i-iLotStart] = KDC3WData[iLinkSp-1].SOKUTEI[i][j];
                    if( KDC3WData[iLinkSp-1].SOKUTEI[i][j] < Min[i-iLotStart] ) Min[i-iLotStart] = KDC3WData[iLinkSp-1].SOKUTEI[i][j];

                    Val[i-iLotStart] += KDC3WData[iLinkSp-1].SOKUTEI[i][j] ;
              }

              Val[i-iLotStart] = Val[i-iLotStart]/KDC3WData[iLinkSp-1].SOKUTEI_NM[i] ;
          }

          str = KDC3WData[iLinkSp-1].LOTNO[i] ;
   ...
   if( KDC1Data[iSp-1].SOKUTEI_NM_CUR != 0 )
   {
         ...
         Min[iRRR] = mina ;
         for( i=0; i< KDC1Data[iSp-1].SOKUTEI_NM_CUR ; i++ ){
              if( KDC1Data[iSp-1].SOKUTEI_M[i] > Max[iRRR] ) Max[iRRR] = KDC1Data[iSp-1].SOKUTEI_M[i];
              if( KDC1Data[iSp-1].SOKUTEI_M[i] < Min[iRRR] ) Min[iRRR] = KDC1Data[iSp-1].SOKUTEI_M[i];
         }

         // 平均値の計算
         for( i=0; i< KDC1Data[iSp-1].SOKUTEI_NM_CUR; i++ )
              Val[iRRR] += KDC1Data[iSp-1].SOKUTEI_M[i] ;
         Val[iRRR] = Val[iRRR]/KDC1Data[iSp-1].SOKUTEI_NM_CUR ;
   }

   str = KDC1Data[iSp-1].LOTNO ;
   if( str.Length() > 5 ) str = str.Delete(1,1);
...
void DrawRireki(TImage *GraphName,int n, char *Pstr, double Max[10], double Min[10], double Val[10], int iCur)
{
    ...
    //最大値、最小値の検索
    MaxY = KDC1Data[SamplePositionIdx-1].MAGEG_MAX ;
    MinY = KDC1Data[SamplePositionIdx-1].MAGEG_MIN;
...
*  create        : 1999.11                                             *
*  update        :                                                     *
*  contents      : KDC3Wから読み込んだﾃﾞｰﾀ中にに指定したロットNo       *
*                :  有るかどうか のチェック                            *
*  written by    : 呉 亜娟(NKK unics Corp.)                            *
***********************************************************************/
int  CheckLotNo(int iSp)
{
   ...
   for( int i=0; i<  KDC3WData[iLinkSp-1].LOTCNT; i++ ){
      if(  KDC1Data[iSp-1].LOTNO ==  KDC3WData[iLinkSp-1].LOTNO[i] )
      {
        iFlag = i ;
        break ;
       }
