...
extern struct structKDC1{
      ...
      double MAGEG_MAX ;        //曲げ強度 規格上
      double MAGEG_MIN ;        //曲げ強度 規格下
      double MAGEG_SPAN;        //曲げ強度 測定ｽﾊﾟﾝ
      int    MAGEG_TAN;         //曲げ強度 単位
      int    SOKUTEI_NM ;      //測定個数
      int    SOKUTEI_SU ;      //測定個数
...
//抗折力履歴作画用テーブル変数 KDC3W
extern struct structKDC3W{
      AnsiString DTKSHIN;          //得意先品番
      AnsiString LOTNO[50];        //ロットNO
      double MAGEG_MAX[50] ;        //曲げ強度 規格上
      double MAGEG_MIN[50] ;        //曲げ強度 規格下
...
extern void KDC3WFormat(void);
extern void AddKDC3W(void);
extern int  CheckLotNo(int ) ;

extern int IsNullSpace(AnsiString);
extern int GetItem(char **buff, char *item) ;
extern int KDC1_Serch01(void);
extern int KDC1_Serch02(void);
extern int KDC1_Serch03(void);
extern void KDC1_Update01(void);

extern long KDC3W_Create01(AnsiString, int, int) ;
extern int  KDC3W_Serch01();
extern void KDC1Format(void);
