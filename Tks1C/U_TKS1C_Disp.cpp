     ...
     if(pEdt2_S[0]->Focused() ||
        ...
        if( KDC1Data[SamplePositionIdx-1].SOKUTEI_NM > 2 )
        {
            ...
            while( 1 ){
                ...
                if( KDC1Data[SamplePositionIdx-1+irec].SOKUTE_KBN  == 1){
                    iFlg = 1;
                    break ;
                 }
             }
     ...
     if(pEdt3_S[0]->Focused() ||
        ...
        // ﾃﾞｰﾀセーブフラグを設定
         iSaveFlag = 1;
          ...
          if( n >= KDC1Data[SamplePositionIdx-1].SOKUTEI_NM  ){
            ...
            //データの更新
             pEdt3[SamplePositionIdx-1]->Text = KDC1Data[SamplePositionIdx-1].SOKUTEI_M[n] ;
             ...
             if( KDC1Data[SamplePositionIdx-1].KEIJYO_KBN != 4 )
                pEdt3_S[0]->Text = FormatFloat("#0.000",KDC1Data[SamplePositionIdx-1].SOKUTEI_S[n] );
             else
                pEdt3_S[0]->Text = "0.000";

             pEdt3_S[1]->Text = FormatFloat("#0.000",KDC1Data[SamplePositionIdx-1].SOKUTEI_H[n] );

             //2003.02.10 E.Takase
             if( KDC1Data[SamplePositionIdx-1].KEIJYO_KBN == 5 ){
                 pEdt3_S[2]->Text = FormatFloat("#0.00",KDC1Data[SamplePositionIdx-1].SOKUTEI_K[n] );
             } else {
                 pEdt3_S[2]->Text = FormatFloat("#0.0",KDC1Data[SamplePositionIdx-1].SOKUTEI_K[n] );
             }
             if( KDC1Data[SamplePositionIdx-1].KEIJYO_KBN == 4 ){
                     pEdt4_S[0]->Text = "0" ;
                     pPnl_S[0]->Caption = "0" ;
...
int iFlg, n;
   ...
   //  FocusがPUSHﾎﾞﾀﾝにいる場合最後ﾎﾟｼﾞｼｮﾝNOに設定
    if(  Form1->BtnF01->Focused() ||
        ...
        if( KDC1Data[iLastPosition-1].SOKUTEI_NM > 2 )
    ...
    // ﾃﾞｰﾀ1にFocusがある
     if(pEdt1_S[0]->Focused() ||
        ...
        if( iFlg == 0 ){
              ...
              // 計測データ数 ＞２の場合
              if( KDC1Data[SamplePositionIdx-irec-1].SOKUTEI_NM > 2 ){
         ...
         iSaveFlag = 1;
          ...
          if( n <= 3){
                ...
                ToDrawGraph(SamplePositionIdx, iOption);
                 ...
                 // データの更新
                 pEdt3[SamplePositionIdx-1]->Text = KDC1Data[SamplePositionIdx-1].SOKUTEI_M[n-1-1] ;
                 pScrl[SamplePositionIdx-1]->Position = n-1 ;
                 pEdt4[SamplePositionIdx-1]->Text = IntToStr( n-1) ;

                 //測定データ明細の表示
                if( KDC1Data[SamplePositionIdx-1].KEIJYO_KBN != 4 )
                    pEdt3_S[0]->Text = FormatFloat("#0.000",KDC1Data[SamplePositionIdx-1].SOKUTEI_S[n-1-1] );
                else
                    pEdt3_S[0]->Text = "0.000";

                pEdt3_S[1]->Text = FormatFloat("#0.000",KDC1Data[SamplePositionIdx-1].SOKUTEI_H[n-1-1] );

                //2003.02.10 E.Takase
                if( KDC1Data[SamplePositionIdx-1].KEIJYO_KBN == 5 ){
                   pEdt3_S[2]->Text = FormatFloat("#0.00",KDC1Data[SamplePositionIdx-1].SOKUTEI_K[n-1-1] );
                } else {
                   pEdt3_S[2]->Text = FormatFloat("#0.0",KDC1Data[SamplePositionIdx-1].SOKUTEI_K[n-1-1] );
                }
                if( KDC1Data[SamplePositionIdx-1].KEIJYO_KBN == 4 ){
                     pEdt4_S[0]->Text = "0" ;
                     pPnl_S[0]->Caption = "0" ;
  ...
  int     errflg=0;
    ...
    if( SamplePositionIdx > 0 && (SamplePositionIdx >= iFirstPosition &&
      ...
      if ( CheckKomoku(SamplePositionIdx, DataNoIdx) ){
       ...
       if(( dBuf > KDC1Data[SamplePositionIdx-1].MAGEG_MAX  ||
           dBuf < KDC1Data[SamplePositionIdx-1].MAGEG_MIN ) &&
           DataNoIdx <= KDC1Data[SamplePositionIdx-1].SOKUTEI_NM_CUR ){
     ...
     if( KDC1Data[SamplePositionIdx-1].SOKUTEI_NM > 2 && DataNoIdx >=3  && errflg != 3){
        int k=0;
        for(int  i=2; i< KDC1Data[SamplePositionIdx-1].SOKUTEI_NM; i++ ){

             if( ( KDC1Data[SamplePositionIdx-1].SOKUTEI_M[i] > KDC1Data[SamplePositionIdx-1].MAGEG_MAX ||
                   KDC1Data[SamplePositionIdx-1].SOKUTEI_M[i] < KDC1Data[SamplePositionIdx-1].MAGEG_MIN )  &&
                  KDC1Data[SamplePositionIdx-1].SOKUTE_KBN == 1 &&
                  DataNoIdx <= KDC1Data[SamplePositionIdx-1].SOKUTEI_NM_CUR  ){
...
int  SaveRegaiDt(void )
{
   ...
   if( KDC1Data[SamplePositionIdx-1].SOKUTEI_NM > 2  &&
       KDC1Data[SamplePositionIdx-1].SOKUTE_KBN == 1 &&  DataNoIdx >  2 )
   {
      ...
      if( fabs(pEdt3_S[iKomoku-1]->Value - a ) > 0.000001){
          ...
          // 計測したデータCount
          if ( KDC1Data[SamplePositionIdx-1].SOKUTEI_NS_CUR < n && iKomoku == 1  )
               KDC1Data[SamplePositionIdx-1].SOKUTEI_NS_CUR = n ;
          else if ( KDC1Data[SamplePositionIdx-1].SOKUTEI_NH_CUR < n && iKomoku == 2  )
                    KDC1Data[SamplePositionIdx-1].SOKUTEI_NH_CUR = n ;
          else if ( KDC1Data[SamplePositionIdx-1].SOKUTEI_NK_CUR < n && iKomoku == 3  )
         ...
         if( MForceCal(SamplePositionIdx, n ) ){
          ...
          //  if ( KDC1Data[SamplePositionIdx-1].SOKUTEI_NM_CUR < n )
          //       KDC1Data[SamplePositionIdx-1].SOKUTEI_NM_CUR = n ;
           }
         }
   }
   return 1;
...
{
     Form1->Pnl5->Caption = FormatFloat("#0.0", KDC1Data[SamplePositionIdx-1].MAGEG_MIN) ;
     Form1->Pnl6->Caption = FormatFloat("#0.0", KDC1Data[SamplePositionIdx-1].MAGEG_MAX) ;
     if(KDC1Data[SamplePositionIdx-1].MAGEG_TAN == 1)
        Form1->Pnl6_1->Caption = "Kgf/c㎡" ;
     else if(KDC1Data[SamplePositionIdx-1].MAGEG_TAN == 2 )
        Form1->Pnl6_1->Caption = " N/c㎡ " ;
     else if( KDC1Data[SamplePositionIdx-1].MAGEG_TAN == 4 )
        Form1->Pnl6_1->Caption = " N/m㎡ " ;
     //2003/07/22 E.Takase 単位追加
     else if( KDC1Data[SamplePositionIdx-1].MAGEG_TAN == 5 )
        Form1->Pnl6_1->Caption = " N    " ;
     else
        Form1->Pnl6_1->Caption = " MPa  " ;

     Form1->Pnl7->Caption = KDC1Data[SamplePositionIdx-1].DTKSCOD ;
     Form1->Pnl8->Caption = KDC1Data[SamplePositionIdx-1].DTKSHIN ;
     Form1->Pnl9->Caption = KDC1Data[SamplePositionIdx-1].LOTNO ;
     Form1->Pnl10->Caption = KDC1Data[SamplePositionIdx-1].HINBAN ;
     Form1->Pnl11->Caption = KDC1Data[SamplePositionIdx-1].ZISCOD ;
     Form1->Pnl12->Caption = KDC1Data[SamplePositionIdx-1].ZISNAM ;

     Form1->Pnl13->Caption = FormatFloat("#0.0", KDC1Data[SamplePositionIdx-1].MAGEG_SPAN) ;
...
void SetRed(int iFlg)
{
   ...
   for( i=0; i<30 ; i++ ){
      ...
      if( KDC1Data[i].SOKUTE_KBN != 1 ) continue ;

      //最初登録のときに規格チェックしない
      if( iFlg == 1 && IsNullSpace( KDC1Data[i].ADDYMD ) ) {
          pEdt1[i]->Font->Color = clBlack;
          pEdt2[i]->Font->Color = clBlack;
          pEdt3[i]->Font->Color = clBlack;
          pEdt4[i]->Font->Color == clBlack;
          continue ;
           ...
           //測定値１ (抵抗）
           if(( KDC1Data[i].SOKUTEI_M[0] >  KDC1Data[i].MAGEG_MAX  ||
                KDC1Data[i].SOKUTEI_M[0] <  KDC1Data[i].MAGEG_MIN ) && ! CheckKomoku(i+1, 1) ){
           ...
           //測定値2 (抵抗）
           if( (KDC1Data[i].SOKUTEI_M[1] >  KDC1Data[i].MAGEG_MAX   ||
                KDC1Data[i].SOKUTEI_M[1] <  KDC1Data[i].MAGEG_MIN)  && ! CheckKomoku(i+1, 2)){
           ...
           if( KDC1Data[i].SOKUTEI_NM > 2 ){
               ...
               k = 0;
               for( j=2; j< KDC1Data[i].SOKUTEI_NM ; j++){

                     //測定値3～ （抵抗）
                    if( ( KDC1Data[i].SOKUTEI_M[j] >  KDC1Data[i].MAGEG_MAX  ||
                          KDC1Data[i].SOKUTEI_M[j] <  KDC1Data[i].MAGEG_MIN ) &&! CheckKomoku(i+1, j+1)){
                         k = 1 ;
                         break ;
                    }
...
int  MForceCal(int iSp,int iDt)
{
  ...
  //円柱の場合
  if(  KDC1Data[iSp-1].KEIJYO_KBN == 4) {
      if( CheckKomoku( iSp, iDt ) == 0  ){

        // kgf/cm**2 || N/cm**2
        if(KDC1Data[iSp-1].MAGEG_TAN == 1  || KDC1Data[iSp-1].MAGEG_TAN == 2 ){

          KDC1Data[iSp-1].SOKUTEI_M[iDt-1] =
               8.0* KDC1Data[iSp-1].SOKUTEI_K[iDt-1]*KDC1Data[iSp-1].SOKUTEI_P[iDt-1]/10 ;
          KDC1Data[iSp-1].SOKUTEI_M[iDt-1] = KDC1Data[iSp-1].SOKUTEI_M[iDt-1]/
                   (3.1415926535*KDC1Data[iSp-1].SOKUTEI_H[iDt-1]*KDC1Data[iSp-1].SOKUTEI_H[iDt-1]*KDC1Data[iSp-1].SOKUTEI_H[iDt-1]/1000.0);
        // N/mm**2
        }else if( KDC1Data[iSp-1].MAGEG_TAN == 4 ){

          KDC1Data[iSp-1].SOKUTEI_M[iDt-1] =
               8.0* KDC1Data[iSp-1].SOKUTEI_K[iDt-1]*KDC1Data[iSp-1].SOKUTEI_P[iDt-1];
          KDC1Data[iSp-1].SOKUTEI_M[iDt-1] = KDC1Data[iSp-1].SOKUTEI_M[iDt-1]/
                   (3.1415926535*KDC1Data[iSp-1].SOKUTEI_H[iDt-1]*KDC1Data[iSp-1].SOKUTEI_H[iDt-1]*KDC1Data[iSp-1].SOKUTEI_H[iDt-1] );
          ...
          KDC1Data[iSp-1].SOKUTEI_M[iDt-1] =
               8.0* KDC1Data[iSp-1].SOKUTEI_K[iDt-1]*KDC1Data[iSp-1].SOKUTEI_P[iDt-1]/10 ;
          KDC1Data[iSp-1].SOKUTEI_M[iDt-1] = KDC1Data[iSp-1].SOKUTEI_M[iDt-1]/
                   (100.0*3.1415926535*KDC1Data[iSp-1].SOKUTEI_H[iDt-1]*KDC1Data[iSp-1].SOKUTEI_H[iDt-1]*KDC1Data[iSp-1].SOKUTEI_H[iDt-1]/1000.0);
        ...
        ret = 1 ;
          ...
          KDC1Data[iSp-1].SOKUTEI_M[iDt-1] = 0.0 ;
          ret = 0;
      }

  //コンミの場合
  }else if(  KDC1Data[iSp-1].KEIJYO_KBN == 5) {
      if( CheckKomoku( iSp, iDt ) == 0  ){
          KDC1Data[iSp-1].SOKUTEI_M[iDt-1] = KDC1Data[iSp-1].SOKUTEI_K[iDt-1];
          ret = 1 ;
      }else{
          KDC1Data[iSp-1].SOKUTEI_M[iDt-1] = 0.0 ;
          ret = 0;
  ...
  //ブロックの場合
      ...
      if( CheckKomoku( iSp, iDt ) == 0  ){
        ...
        if(KDC1Data[iSp-1].MAGEG_TAN == 1  || KDC1Data[iSp-1].MAGEG_TAN == 2 ){
           ...
           KDC1Data[iSp-1].SOKUTEI_M[iDt-1] =
               3.0* KDC1Data[iSp-1].SOKUTEI_K[iDt-1]*KDC1Data[iSp-1].SOKUTEI_P[iDt-1]*0.1 ;
           KDC1Data[iSp-1].SOKUTEI_M[iDt-1] = KDC1Data[iSp-1].SOKUTEI_M[iDt-1]/
                   (2.0*KDC1Data[iSp-1].SOKUTEI_H[iDt-1]*KDC1Data[iSp-1].SOKUTEI_S[iDt-1]*KDC1Data[iSp-1].SOKUTEI_S[iDt-1]/1000.0);

        // N/mm**2
        }else if( KDC1Data[iSp-1].MAGEG_TAN == 4 ){
           KDC1Data[iSp-1].SOKUTEI_M[iDt-1] =
               3.0* KDC1Data[iSp-1].SOKUTEI_K[iDt-1]*KDC1Data[iSp-1].SOKUTEI_P[iDt-1];
           KDC1Data[iSp-1].SOKUTEI_M[iDt-1] = KDC1Data[iSp-1].SOKUTEI_M[iDt-1]/
                   (2.0*KDC1Data[iSp-1].SOKUTEI_H[iDt-1]*KDC1Data[iSp-1].SOKUTEI_S[iDt-1]*KDC1Data[iSp-1].SOKUTEI_S[iDt-1] );
          ...
          KDC1Data[iSp-1].SOKUTEI_M[iDt-1] =
               3.0* KDC1Data[iSp-1].SOKUTEI_K[iDt-1]*KDC1Data[iSp-1].SOKUTEI_P[iDt-1]*0.1 ;
          KDC1Data[iSp-1].SOKUTEI_M[iDt-1] = KDC1Data[iSp-1].SOKUTEI_M[iDt-1]/
                   (200.0*KDC1Data[iSp-1].SOKUTEI_H[iDt-1]*KDC1Data[iSp-1].SOKUTEI_S[iDt-1]*KDC1Data[iSp-1].SOKUTEI_S[iDt-1]/1000.0);
        }
  ...
  // 抵抗率計測数
  if( ret){
      if ( KDC1Data[iSp-1].SOKUTEI_NM_CUR < iDt )
           KDC1Data[iSp-1].SOKUTEI_NM_CUR = iDt ;
  }
...
int CheckKomoku(int iSp, int iDt)
{
  ...
  //円柱
  if( KDC1Data[iSp-1].KEIJYO_KBN == 4 ) {
     if( KDC1Data[iSp-1].SOKUTEI_NP_CUR == 0  &&
         KDC1Data[iSp-1].SOKUTEI_NH_CUR == 0  &&
         KDC1Data[iSp-1].SOKUTEI_NK_CUR == 0  ){
         ret = 1 ;
         return ret;
     }

     if(   KDC1Data[iSp-1].SOKUTEI_H[iDt-1] <= 0.0000001 ||
           KDC1Data[iSp-1].SOKUTEI_K[iDt-1] <= 0.0000001 ||
           KDC1Data[iSp-1].SOKUTEI_P[iDt-1] <= 0.0000001    ) {
           ret = 1;
          return ret ;
     }
  //コンミの場合
  } else if ( KDC1Data[iSp-1].KEIJYO_KBN == 5 ) {
     if( KDC1Data[iSp-1].SOKUTEI_NS_CUR == 0  &&
         KDC1Data[iSp-1].SOKUTEI_NK_CUR == 0  ){
         ret = 1 ;
         return ret;
     }

     if(   KDC1Data[iSp-1].SOKUTEI_S[iDt-1] <= 0.0000001 ||
           KDC1Data[iSp-1].SOKUTEI_K[iDt-1] <= 0.0000001    ) {
           ret = 1;
          return ret ;
     }
  // 長方体の場合
     ...
     if( KDC1Data[iSp-1].SOKUTEI_NP_CUR == 0  &&
         KDC1Data[iSp-1].SOKUTEI_NS_CUR == 0  &&
         KDC1Data[iSp-1].SOKUTEI_NH_CUR == 0  &&
         KDC1Data[iSp-1].SOKUTEI_NK_CUR == 0  ){
         ret = 1 ;
         return ret;
     ...
     if(   KDC1Data[iSp-1].SOKUTEI_S[iDt-1] <= 0.0000001 ||
           KDC1Data[iSp-1].SOKUTEI_H[iDt-1] <= 0.0000001 ||
           KDC1Data[iSp-1].SOKUTEI_K[iDt-1] <= 0.0000001 ||
           KDC1Data[iSp-1].SOKUTEI_P[iDt-1] <= 0.0000001    ) {
           ret = 1;
          return ret ;
...
int IsKeisokuEnd(void)
{
  ...
  for( i=0; i<30; i++ ){
         ...
         if( (KDC1Data[i].SOKUTEI_NM_CUR < KDC1Data[i].SOKUTEI_NM ) &&

              KDC1Data[i].SOKUTE_KBN ){
