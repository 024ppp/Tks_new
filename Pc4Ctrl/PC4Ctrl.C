...
#include "PC4Ctrl.h"         /* specific to this program              */
...
WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	wsprintf(DirName, "%s\\%s", DirName, "PC4Ctrl.ini");
    ...
    int             nCmdShow)           /* Param for first ShowWindow() call. */
{
    ...
    hWnd = CreateWindow(
        ...
        "TRIS ： 中間検査(PC4)",		/* Text for window title bar.         */
...
//	char *Tstring;
	...
	switch (message) {
		...
		case WM_CREATE:
			...
			sprintf(SharedBuff, "%s\\LogPC4Ctrl.dat", tempo);
			//2009.03.13 MTS E.Takase [001]  ↑
		...
		case WM_COM1:	// RS232C 処理 (COM1)
			...
			if (AcceptFlag == 1 && rtn > 0) {
				...
				if (strchr(BarString, 0x0D) != NULL) {
					...
					//20160708 文字が含まれているかどうかチェック
					//MyComBufLen = strlen(MyComBuf);
					iPos = FindString(MyComBuf,"m");
					if ( iPos > 0 ) {
					//Found
						/*spos = iPos-1;
						epos = MyComBufLen;*/
						...
						T_Double1 = atof(sResult);

						spos = 9;
						epos = 14;
						iRtn = GetMidString(MyComBuf, spos, epos, sResult);
						T_Double2 = atof(sResult);

						//Tks17側の読み取りフォーマットに合わせておく
						sprintf(MyComBuf, "%.3f %.3f", T_Double1, T_Double2);

						//20160708 CurrentNoを10→9
						//if (CurrentNo == 10)		
						if (CurrentNo == 9)
