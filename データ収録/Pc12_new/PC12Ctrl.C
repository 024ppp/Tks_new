...
char TTYFlow[2][10];		// NONE, DTRDSR, RTSCTS, HARDWARE, XONXOFF, SOFTWARE
...
#if 1	// 2011.06.08 Y.Sano Add start
	// Get TTY Flow Ctrl Message
	GetPrivateProfileString("COM1", "FLOWCTRL", "Not Define", tmsg, 16, DirName);
	if (strncmp(tmsg, "Not Define", 10) == 0) {
		MessageBox(NULL, "TTY Flow Ctrlを定義して下さい", "ReadINIFile", MB_OK);
		return FALSE;
	} else 
		wsprintf(TTYFlow[0], "%s", tmsg);
...
#if 1	// 2011.06.08 Y.Sano Add start
		// Get TTY Flow Ctrl Message
		GetPrivateProfileString("COM2", "FLOWCTRL", "Not Define", tmsg, 16, DirName);
		if (strncmp(tmsg, "Not Define", 10) == 0) {
			MessageBox(NULL, "TTY Flow Ctrlを定義して下さい", "ReadINIFile", MB_OK);
			return FALSE;
		} else 
			wsprintf(TTYFlow[1], "%s", tmsg);
#endif	// 2011.06.08 Y.Sano Add end
