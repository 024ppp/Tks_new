...
Next1:		
        ...
        case WM_CHAR:
			if (wParam == 0x0D) { // Process a carriage return.          
...
int ChangeChannel(int CNo, int Opt)
{
	...
	if ((SocketChannel[0] < 0 && SocketChannel[1] < 0) || Opt == 1)
		GetChannelData();
...
void ReadChannelDataAgain()
{
	...
	if (SocketChannel[0] < 0 && SocketChannel[1] < 0)
		return;
	...
	if (SocketChannel[0] >= 0 && SocketChannel[1] < 0) {
		CurrentSocket = 0;
		ChangeChannel(SocketChannel[0], 1);
		//GetChannelData();
	}

	if (SocketChannel[1] >= 0 && SocketChannel[0] < 0) {
		CurrentSocket = 1;
		ChangeChannel(SocketChannel[1], 1);
		//GetChannelData();
	}
