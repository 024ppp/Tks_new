...
void DetectProc1();
int ChangeChannel(int CNo, int Opt);
int GetChannelData();
        ...
        LONG lParam)              /* additional information          */
{
	...
	switch (message) {
		...
		case WM_CREATE:
			...
			if (Testfn == NULL) {
				MessageBox(hWnd, "ログファイル作成エラー", "", MB_OK);
				return 0;
			}
			...
			if (!OpenConnection(hWnd)) {
				...
				if (TTYFlag == 0) {
					MessageBox(hWnd, "通信接続エラー", "TTY", MB_OK);
					return 0;					
				} 
			}

			// Multiplexer 初期化
			//2009.03.11 MTS E.Takase [ポート変更] ↓
			MP_Init('4');
			//2009.03.11 MTS E.Takase [ポート変更] ↑
			MP_Init('1');
		...
		case WM_COMMNOTIFY:	// RS232C 処理
			...
			fprintf(Testfn, "RS232C Len = %d, Buf=%s\n", rtn, BarString);
			fprintf(Testfn, "-------------------------------\n");
			...
			if (rtn > 1) {
				...
				if (strchr(BarString, 0x0D) != NULL) {
					if (BarString[0] >= '0' && BarString[0] <= '4') {
						cc = BarString[0] - 0x30;
						strcpy(D_Data[cc].Buffer, BarString+1);
...
// Select Channel
int ChangeChannel(int CNo, int ClearFlag)
{
	BYTE TTYChar[10];
	BOOL rtn;
	int i;

	if (ClearFlag == 1) {	// Clear Connected Buffer 
		...
		//2009.03.25 MTS E.Takase [すべてクリア] ↓
		MP_Clear();		// Clear Memory	
		//2009.03.25 MTS E.Takase [すべてクリア] ↑
	}

	//	MP_Clear();		// Clear Memory		1999.12.08
...
AcceptAgain:
	...
	if (Socket_Server_Accept(SocketNo) > 0) {
		while (1) {	// 無限ループ
			...
			if (rtn > 0) {
				...
				if (strlen(SocketBuf1) > 3) {
					...
					if (strncmp(SocketBuf1, "CLOSE", 5) == 0) {
						if (ChannelNo >= 0) {
							...
							//2009.03.25 MTS E.Takase [ポート変更] ↓
							//ChangeChannel(0, 0);		// 1999.12.08
							ChangeChannel(3, 0);		// 1999.12.08
							//2009.03.25 MTS E.Takase [ポート変更] ↑
							EnterCriticalSection(&G_InterfaceBox);
							D_Data[ChannelNo].SocketNo = 0;
							ChannelNo = -1;
				...
				Socket_Close(SocketNo);
				goto AcceptAgain;
			}
		}
	} else {
		MessageBox(MainhWnd, "ソケットエラー", "", MB_OK);
	}
	...
	case 2:
		...
		break;
	case 3:
		ExitThread(GetExitCodeThread(hThread3, &DW3));
		break;
	case 4:
		ExitThread(GetExitCodeThread(hThread4, &DW4));
		break;
	case 5:
		ExitThread(GetExitCodeThread(hThread5, &DW5));
		break;				
	}
}
...
int DataDecode(int ChannelNo, char *DataBuf, double *T_Double1, double *T_Double2)
{
	...
	if (ChannelNo == 1 || ChannelNo == 2) {
		...
		*T_Double1 = 0.0;
		...
		//2009.03.02 MTS E.Takase [ポート変更] ↓
		//if (ChannelNo == 0) {
		if (ChannelNo == 3) {
		//2009.03.02 MTS E.Takase [ポート変更] ↑
			// Format: "1)MAX LOAD POINT"の次行
			if (Channel0_ReadFlag == 1) {
				*T_Double1 = atof(DataBuf+1);
				*T_Double2 = 0;
				Channel0_ReadFlag = 0;
				fprintf(Testfn, "Found Data=%s\n", DataBuf);		// LogFile
				if( *T_Double1 < 1.0 )   return 0;					// データが1以下の時は読み取らない
				return 1;
			} else {
				//if (FindString(DataBuf, "1)MAX LOAD POINT")) {
				if (FindString(DataBuf, "POINT")) {
					fprintf(Testfn, "Found=%s\n", DataBuf);
