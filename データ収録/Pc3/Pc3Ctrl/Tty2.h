...
#if 1	// 2011.06.08 Y.Sano Add
#define FC_NONE			0x00
#define FC_HARDWARE		0x03
#endif	// 2011.06.08 Y.Sano Add
...
LRESULT NEAR CreateTTYInfo(int, HWND ) ;
BOOL NEAR DestroyTTYInfo(int, HWND ) ;
BOOL NEAR ResetTTYScreen(int, HWND, NPTTYINFO ) ;
BOOL NEAR KillTTYFocus(int, HWND ) ;
//BOOL NEAR PaintTTY( HWND ) ;
BOOL NEAR SetTTYFocus(int, HWND ) ;
//BOOL NEAR ScrollTTYHorz( HWND, WORD, WORD ) ;
//BOOL NEAR ScrollTTYVert( HWND, WORD, WORD ) ;
//BOOL NEAR SizeTTY( HWND, WORD, WORD ) ;
BOOL NEAR ProcessTTYCharacter(int, HWND, BYTE ) ;
BOOL NEAR WriteTTYBlock(int, HWND, LPSTR, int ) ;
int NEAR ReadCommBlock(int, HWND, LPSTR, int ) ;
BOOL NEAR WriteCommByte(int, HWND, BYTE ) ;
BOOL NEAR MoveTTYCursor(int, HWND ) ;
BOOL NEAR OpenConnection(int, HWND ) ;
BOOL NEAR SetupConnection(int, HWND ) ;
BOOL NEAR CloseConnection(int, HWND ) ;
int NEAR ProcessCOMMNotification(int, HWND, WPARAM, LPARAM) ;
