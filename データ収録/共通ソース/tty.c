...
extern char TTYFlow[10];		// NONE, DTRDSR, RTSCTS, HARDWARE, XONXOFF, SOFTWARE
...
//   NPTTYINFO   npTTYInfo ;
   ...
   if (strncmp(TTYPort, "COM1", 4) == 0)
	   PORT( npTTYInfo )      = 1 ;				
   if (strncmp(TTYPort, "COM2", 4) == 0)
	   PORT( npTTYInfo )      = 2 ;
   if (strncmp(TTYPort, "COM3", 4) == 0)
	   PORT( npTTYInfo )      = 3 ;
   if (strncmp(TTYPort, "COM4", 4) == 0)
	   PORT( npTTYInfo )      = 4 ;

   switch (TTYBuadRate) {
		case 110:
			BAUDRATE( npTTYInfo )      = CBR_110 ;
			break;
		case 300:
			BAUDRATE( npTTYInfo )      = CBR_300 ;
			break;
		case 600:
			BAUDRATE( npTTYInfo )      = CBR_600 ;
			break;
		case 1200:
			BAUDRATE( npTTYInfo )      = CBR_1200 ;
			break;
		case 2400:
			BAUDRATE( npTTYInfo )      = CBR_2400 ;
			break;
		case 4800:
		...
		case 9600:
			...
			break;
		case 14400:
			BAUDRATE( npTTYInfo )      = CBR_14400 ;
			break;
		case 19200:
			BAUDRATE( npTTYInfo )      = CBR_19200 ;
			break;
		case 38400:
			BAUDRATE( npTTYInfo )      = CBR_38400 ;
			break;
		case 56000:
			BAUDRATE( npTTYInfo )      = CBR_56000 ;
			break;
		case 128000:
			BAUDRATE( npTTYInfo )      = CBR_128000 ;
			break;
		case 256000:
			BAUDRATE( npTTYInfo )      = CBR_256000 ;
			break;
   }
   BYTESIZE( npTTYInfo )      = TTYDataBits ;
   FLOWCTRL( npTTYInfo )      = FC_RTSCTS ;
   if (strncmp(TTYFlow, "NONE", 4) == 0)
	   FLOWCTRL( npTTYInfo )     = FC_NONE ;
   if (strncmp(TTYFlow, "DTRDSR", 6) == 0)
	   FLOWCTRL( npTTYInfo )     = FC_DTRDSR ;
   if (strncmp(TTYFlow, "RTSCTS", 6) == 0)
	   FLOWCTRL( npTTYInfo )     = FC_RTSCTS ;
   if (strncmp(TTYFlow, "HARDWARE", 8) == 0)
	   FLOWCTRL( npTTYInfo )     = FC_HARDWARE ;
   if (strncmp(TTYFlow, "XONXOFF", 7) == 0)
	   FLOWCTRL( npTTYInfo )     = FC_XONXOFF ;
   if (strncmp(TTYFlow, "SOFTWARE", 8) == 0)
	   FLOWCTRL( npTTYInfo )     = FC_XONXOFF ;
   
   if (strncmp(TTYParity, "NONE", 4) == 0)
	   PARITY( npTTYInfo )    = NOPARITY;				
   if (strncmp(TTYParity, "EVEN", 4) == 0)
	   PARITY( npTTYInfo )    = EVENPARITY;
   if (strncmp(TTYParity, "ODD", 3) == 0)
	   PARITY( npTTYInfo )    = ODDPARITY;
   if (strncmp(TTYParity, "MARK", 4) == 0)
	   PARITY( npTTYInfo )    = MARKPARITY;
   if (strncmp(TTYParity, "SPACE", 4) == 0)
	   PARITY( npTTYInfo )    = SPACEPARITY;
   if (TTYStopBits == 1)
	   STOPBITS( npTTYInfo )  = ONESTOPBIT ;
   if (TTYStopBits == 1.5)
	   STOPBITS( npTTYInfo )  = ONE5STOPBITS ;
   if (TTYStopBits == 2)
...
int NEAR ProcessCOMMNotification( HWND hWnd, WPARAM wParam, LPARAM lParam )
{
...
BOOL NEAR ProcessTTYString(HWND hWnd, BYTE *bOut, int nLen )
{
//   NPTTYINFO  npTTYInfo ;
...
//      return ( FALSE ) ;
   ...
   WriteCommString(hWnd, bOut, nLen ) ;
   if (LOCALECHO( npTTYInfo))
      WriteTTYBlock(hWnd, bOut, nLen ) ;
...
BOOL NEAR WriteCommString(HWND hWnd, BYTE *bByte, int nLen )
{
#ifdef WIN32
   BOOL        fWriteStat ;
   DWORD       dwBytesWritten ;
...
#ifdef WIN32
   fWriteStat = WriteFile( COMDEV( npTTYInfo ), (LPSTR) bByte, nLen,
                           &dwBytesWritten, &WRITE_OS( npTTYInfo) ) ;
   if (!fWriteStat && (GetLastError() == ERROR_IO_PENDING))
   {
      // wait for a second for this transmission to complete
...
#else
   WriteComm( COMDEV( npTTYInfo), (LPSTR) bByte, nLen ) ;
#endif
   return ( TRUE ) ;
...
DWORD FAR PASCAL CommWatchProc( LPSTR lpData )
{
   ...
   while ( CONNECTED( npTTYInfo ) )
   {
      ...
      if ((dwEvtMask & EV_RXCHAR) == EV_RXCHAR)
      {
         ...
         //WaitForSingleObject( POSTEVENT( npTTYInfo ), 500) ;

		 Sleep(100);		// 1999.10.15
