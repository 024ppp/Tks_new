...
void __fastcall TForm4::QRLabel9Print(TObject *sender, AnsiString &Value)
{
    ...
    if ( Form4->Query1->FieldValues["KOJOKBN"] == 0 ) {
        Value = "–{)" + Form4->Query1->FieldValues["LOTNO"];
    }
    else if ( Form4->Query1->FieldValues["KOJOKBN"] == 1 ) {
        Value = "L)" + Form4->Query1->FieldValues["LOTNO"];
    }
    else {
        Value = "H)" + Form4->Query1->FieldValues["LOTNO"];
    }
}
//---------------------------------------------------------------------------
//‰ºŒÀ’l`ãŒÀ’l
