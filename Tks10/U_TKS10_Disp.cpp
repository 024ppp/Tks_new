...
*  file   name   : U_TKS10_DISP.cpp                                    *
*  create        : 1999.10.20                                          *
*  update        :                                                     *
*  contents      : ﾘｰﾄﾞ線長さ寸法測定GUI処理用関数                     *
   ...
   if(  Form1->edttlno->Focused()){
    ...
    // ﾃﾞｰﾀ２にFocusがある
     if( pEdt2[SamplePositionIdx-1]->Focused()){
         ...
         //計測データ＞２の場合
        if( KD01Data[SamplePositionIdx-1].SOKUTEI_NM > 2 )
        {
           ...
           while( 1 ){
               irec +=1 ;
               if( KD01Data[SamplePositionIdx-1+irec].SOKUTE_KBN  == 1){
               iFlg = 1;
                break ;}
           }
    ...
    if( pEdt3[SamplePositionIdx-1]->Focused() ||
        ...
        pScrl[SamplePositionIdx-1]->Focused() )   {
         ...
         CheckInKikaku(1);
          ...
          if( n >= KD01Data[SamplePositionIdx-1].SOKUTEI_NM  ){
               ...
               if( pEdt3[iLastPosition-1]->Focused() ||
                 ...
                 //データの更新
                 pEdt3[SamplePositionIdx-1]->Text = KD01Data[SamplePositionIdx-1].SOKUTEICHI[n] ;
...
int iFlg;
   ...
   //  FocusがPUSHﾎﾞﾀﾝにいる場合最後ﾎﾟｼﾞｼｮﾝNOに設定
    if(  Form1->BtnF01->Focused() ||
        ...
        if( KD01Data[iLastPosition-1].SOKUTEI_NM > 2 ) {
   ...
   if( pEdt1[SamplePositionIdx-1]->Focused()){
        ...
        if( iFlg == 0 ){
              ...
              iSaveFlag = 1;
              // 計測データ数 ＞２の場合
              if( KD01Data[SamplePositionIdx-irec-1].SOKUTEI_NM > 2 ){
        ...
        pScrl[SamplePositionIdx-1]->Focused() )   {
          ...
          if( n <= 3){
                ...
                ToDrawGraph(SamplePositionIdx, iOption);
                 ...
                 // データの更新
                 pEdt3[SamplePositionIdx-1]->Text = KD01Data[SamplePositionIdx-1].SOKUTEICHI[n-1-1] ;
...
/***********************************************************************
*  function name : void CheckInKikaku(void )                           *
*                  +--------------------------------------+            *
*          引  数  |   なし     |                         |            *
*                  +--------------------------------------+            *
*                  +--------------------------------------+            *
...
void CheckInKikaku(int iBeep)
{
  ...
  int errflg ;
    ...
    if( SamplePositionIdx > 0 && (SamplePositionIdx >= iFirstPosition &&
       ...
       if( ( dBuf > KD01Data[SamplePositionIdx-1].LD_LNG_MAX  ||
             dBuf < KD01Data[SamplePositionIdx-1].LD_LNG_MIN  ) &&
             KD01Data[SamplePositionIdx-1].SOKUTE_KBN == 1 &&
             DataNoIdx <=KD01Data[SamplePositionIdx-1].SOKUTEI_NM_CUR  ){
     ...
     if( errflg && iBeep ) {
      ...
      if( KD01Data[SamplePositionIdx-1].SOKUTEI_NM > 2 && DataNoIdx >=3 && errflg != 3){

        int k=0;
        for(int  i=2; i< KD01Data[SamplePositionIdx-1].SOKUTEI_NM; i++ ){

             if( ( KD01Data[SamplePositionIdx-1].SOKUTEICHI[i] > KD01Data[SamplePositionIdx-1].LD_LNG_MAX  ||
                  KD01Data[SamplePositionIdx-1].SOKUTEICHI[i] < KD01Data[SamplePositionIdx-1].LD_LNG_MIN ) &&
                  KD01Data[SamplePositionIdx-1].SOKUTE_KBN == 1 &&
                  DataNoIdx <=KD01Data[SamplePositionIdx-1].SOKUTEI_NM_CUR ){
...
void SaveRegaiDt(void )
{
   ...
   if( KD01Data[SamplePositionIdx-1].SOKUTEI_NM > 2  &&
       KD01Data[SamplePositionIdx-1].SOKUTE_KBN == 1 &&  DataNoIdx >  2 )
   {
       ...
       if( fabs(pEdt3[SamplePositionIdx-1]->Value - KD01Data[SamplePositionIdx-1].SOKUTEICHI[n-1]) > 0.0000001 ){
           ...
           // 計測したデータ数をｶﾝトー
          if ( KD01Data[SamplePositionIdx-1].SOKUTEI_NM_CUR < n )
               KD01Data[SamplePositionIdx-1].SOKUTEI_NM_CUR = n ;
...
void DisplayInfo(void)
{
     ...
     Form1->Pnl5->Caption = FormatFloat("#0.00", KD01Data[SamplePositionIdx-1].LD_LNG_MIN) ;
     Form1->Pnl6->Caption = FormatFloat("#0.00", KD01Data[SamplePositionIdx-1].LD_LNG_MAX) ;
     Form1->Pnl7->Caption = KD01Data[SamplePositionIdx-1].DTKSCOD ;
     Form1->Pnl8->Caption = KD01Data[SamplePositionIdx-1].DTKSHIN ;
     Form1->Pnl9->Caption = KD01Data[SamplePositionIdx-1].LOTNO ;
     Form1->Pnl10->Caption = KD01Data[SamplePositionIdx-1].HINBAN ;
     Form1->Pnl11->Caption = KD01Data[SamplePositionIdx-1].ZISCOD ;
     Form1->Pnl12->Caption = KD01Data[SamplePositionIdx-1].ZISNAM ;
}
...
void SetRed(int iFlg)
{
   ...
   for( i=0; i<30 ; i++ ){
      ...
      if( iFlg == 1 && IsNullSpace( KD01Data[i].ADDYMD )) {
          pEdt1[i]->Font->Color = clBlack;
          pEdt2[i]->Font->Color = clBlack;
          pEdt3[i]->Font->Color = clBlack;
          pEdt4[i]->Font->Color == clBlack;
      ...
      if( KD01Data[i].SOKUTE_KBN == 1 ){
           ...
           //測定値１
           if( ( KD01Data[i].SOKUTEICHI[0] >  KD01Data[i].LD_LNG_MAX ||
                 KD01Data[i].SOKUTEICHI[0] <  KD01Data[i].LD_LNG_MIN ) &&
                 KD01Data[i].SOKUTEI_NM_CUR>=1 ){
           ...
           //測定値2
           if( ( KD01Data[i].SOKUTEICHI[1] >  KD01Data[i].LD_LNG_MAX ||
                 KD01Data[i].SOKUTEICHI[1] <  KD01Data[i].LD_LNG_MIN  ) &&
                 KD01Data[i].SOKUTEI_NM_CUR>=2 ){
           ...
           if( KD01Data[i].SOKUTEI_NM > 2 ){
               ...
               k = 0;
               for( j=2; j< KD01Data[i].SOKUTEI_NM ; j++){

                     //測定値3～
                    if(( KD01Data[i].SOKUTEICHI[j] >  KD01Data[i].LD_LNG_MAX ||
                        KD01Data[i].SOKUTEICHI[j] <  KD01Data[i].LD_LNG_MIN ) &&
                        KD01Data[i].SOKUTEI_NM_CUR>=j ){
                         k = 1 ;
                         break ;
                    }
