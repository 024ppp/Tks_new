...
//---------------------------------------------------------------------------
//RS232C通信用変数
int iRS232C = 1 ;           // 計測機器と接続するかどうか
int iChNo = 1;              // RS232Cと接続しているチャネル番号
int KikiType =1;            // 実行マシン名 1:Pc1 2:PC2 3:PC3 4:PC4
int ret_232=0;              // 計測機器と正しく接続しているかどうか
...
int  iShareFlag = 0 ;                           // RS232C接続状況
int  ShareFlag = 0 ;                            // RS232Cと正しく接続したかどうかの指示フラグ

//---------------------------------------------------------------------------
// KD01構造体変数
structKD01 KD01Data[30];      //リード線長さﾃﾞｰﾀﾌｧｲﾙ  KD01

//KD03W構造体変数
structKD03W KD03WData[30];    //リード線長さ計測履歴ﾃﾞｰﾀﾜｰｸﾌｧｲﾙ  KD03W


//---------------------------------------------------------------------------
//画面操作用変数
int iRec_Kd01 ;             //一覧検索結果件数
...
int CurSampChg;             // ｶﾚﾝﾄｻﾝﾌﾟﾙﾎﾟｼﾞｼｮﾝNOの計測値を更新されるかどうか
int CurDataChg;             // ｶﾚﾝﾄ計測ﾃﾞｰﾀを更新されるかどうか
                            // 更新した場合 =1   更新しない場合 =0
int iSaveFlag= 0;           // ScrollbarのChangeイベントで使う変数

int CurBackFlg =0 ;
int HaveSokutei = 0;        // 指定した検査予定日とﾄﾚｰNOで検索した結果に計測するﾚｺｰﾄﾞがあるかどうか
...
AnsiString MSG_12 = "計測機器の応答がありません。　検索及びキーボードからの入力は使用可能です。" ;
AnsiString MSG_13 = "計測機器は他のパソコンで使用しています。　検索及びキーボードからの入力は使用可能です。" ;
...
long  Kd03W_Create01(AnsiString, int, int) ;
int  Kd03W_Serch01();
 ...
 // [Data]
     ...
     if (iShareFlag == 2) {
        ...
        if( nNumber > 0 &&  strncmp( SharedBuff, "OK", 2) != 0 &&
           ...
           if( DataNoIdx == 1 ) {
               //測定値１のセット
               pEdt1[SamplePositionIdx-1]->Text = FormatFloat("#0.00", dVal);

           }else if (DataNoIdx == 2 ){
               //測定値２のセット
               pEdt2[SamplePositionIdx-1]->Text = FormatFloat("#0.00", dVal);

           }else{
               // 測定値NO＞２の場合ﾃﾞｰﾀのセット
               pEdt3[SamplePositionIdx-1]->Text = FormatFloat("#0.00", dVal);
           }
           //RS232C読み込みフラグ 1
           RSInputFlag = 1;
...
void ExecF1(void)
{
   if( Form1->edtymd->Focused() || Form1->edttlno->Focused() ){
      ...
      //リード線長さ寸法ﾃﾞｰﾀﾌｧｲﾙからデータを抽出
      iRec_Kd01 =  Kd01_Serch01() ;
      if( iRec_Kd01 <= 0 ) {
          StatBarClear( MSG_02);
      }else{
          StatBarClear( " ");
...
void __fastcall TForm1::BtnF07Click(TObject *Sender)
{
   ...
   iKskStart = 0 ;
    ...
    // リード線長さ寸法ﾃﾞｰﾀﾌｧｲﾙからﾃﾞｰﾀを抽出  KD01
    int iRtn = Kd01_Serch02();
    if( iRtn <= 0 ) {
    ...
    // 計測値の変更があったかどうかの指示フラグ
     ...
     ////KD03W履歴ﾃﾞｰﾀテーブルの作成
     //Kd03W_Create01(KD01Data[0].KENSA_YMD, KD01Data[0].TOLEY_NO, LOTMAX);
     ////if( iRec >0 ){
     //
     //   //KD03W履歴ﾃﾞｰﾀﾌｧｲﾙを読み込む
     //   i  =  Kd03W_Serch01();
     //// }
	 ...
	 if( CheckBoxRireki->Checked ){
	     ...
	     //KD03W履歴ﾃﾞｰﾀテーブルの作成
         //2002/11/25 E.Takase
	     //int iRec = Kd03W_Create01(KD01Data[0].KENSA_YMD, KD01Data[0].TOLEY_NO, LOTMAX);
	     int iRec = Kd03W_Create01(strVal01, strVal02.ToIntDef(0), LOTMAX);
	     if( iRec >0 ){
          ...
          if( KD01Data[i].SAMPLE_P == -1 )
          {
             ...
             //  IsEnabledEdt( pEdt4[i], 0 );
              ...
              //  pScrl[i]->Position = 0 ;
                ...
                //ﾃﾞｰﾀ１、ﾃﾞｰﾀ２の設定
               pEdt1[i]->Text =  FormatFloat("#0.00",KD01Data[i].SOKUTEICHI[0] );
               pEdt2[i]->Text =  FormatFloat("#0.00",KD01Data[i].SOKUTEICHI[1] );
               pPnlN[i]->Caption = IntToStr( KD01Data[i].SOKUTEI_NM) ;

               // 例外ﾃﾞｰﾀ の設定
               if( KD01Data[i].SOKUTEI_NM > 2 )
               {
                 pEdt3[i]->Text =  FormatFloat("#0.00",KD01Data[i].SOKUTEICHI[2] );
                 pEdt4[i]->Text =  IntToStr(3);
               }

               if(  KD01Data[i].SOKUTE_KBN == 1 ){
                   // ﾃﾞｰﾀ１、ﾃﾞｰﾀ２ｺﾝﾄﾛｰﾙ
                  ...
                  if( KD01Data[i].SOKUTEI_NM > 2 )
                  {
                    ...
                    pScrl[i]->Min = 3;
                    pScrl[i]->Position = 3 ;
                    pScrl[i]->Max = KD01Data[i].SOKUTEI_NM;
                  ...
                  iLastPosition = KD01Data[i].SAMPLE_P ;
      ...
      TShiftState Shift)
{
     ...
     switch(Key)
     {
         ...
         case VK_F7:
                       //検索部にフォーカスがある場合、検索実行
                       //2002/11/18 フォーカスのチェックはずした E.Takaase
      ...
      TShiftState Shift)
{
     ...
     switch(Key)
     {
         ...
         case VK_F7:
                      ...
                      //2002/11/18 検索部にフォーカスをセット E.Takaase
                      //if( edtymd->Focused() || edttlno->Focused() || CheckBoxRireki->Focused() ){
                      //    BtnF07Click(Sender);
                      //}
                      ////検索部以外にフォーカスがある場合、検索部にフォーカスをセット
...
void __fastcall TForm1::FormCreate(TObject *Sender)
{
  char DirName[256], FileName[256], tempo[64];
     ...
     if (iRS232C) {
      ...
      // 工場区分を読み込む
      GetPrivateProfileString("MAIN", "KOJOKBN", "", tempo, 64, FileName);	//Seed値
      iKojoKbn = atoi(tempo);
...
void __fastcall TForm1::SetInit(int flag)
{
     int i ;
      ...
      for( i=0; i<30; i++ ){
          InitEdt(pEdt1[i], "0.00");
          InitEdt(pEdt2[i], "0.00");
          InitEdt(pEdt3[i], "0.00");
          InitEdt(pEdt4[i], "0");
          InitPnl(pPnlN[i], "0") ;
      }
       ...
       ( SamplePositionIdx >= iFirstPosition && SamplePositionIdx <= iLastPosition ) &&
         DataNoIdx > 2   ){
          ...
          pEdt3[SamplePositionIdx-1]->Text = FormatFloat("#0.00",
                                      KD01Data[SamplePositionIdx-1].SOKUTEICHI[pos-1] );
...
void __fastcall TForm1::Edt01_1Change(TObject *Sender)
{
 ...
 //       TNumberEdit *Edt = (TNumberEdit *)Sender;
        ...
        CurDataChg = 1;
         ...
         // 計測したデータCount
        if ( KD01Data[SamplePositionIdx-1].SOKUTEI_NM_CUR < DataNoIdx ){
             KD01Data[SamplePositionIdx-1].SOKUTEI_NM_CUR = DataNoIdx ;
        }
   }
}
  ...
  if( iKskStart ) {
     ...
     if( n >= 3 && n <= KD01Data[SamplePositionIdx-1].SOKUTEI_NM ){
        if( n == pScrl[SamplePositionIdx-1]->Position ) return ;
         ...
         pEdt3[SamplePositionIdx-1]->Text = FormatFloat("#0.00",KD01Data[SamplePositionIdx-1].SOKUTEICHI[n-1] );
...
void __fastcall TForm1::SetMEMO(void)
{
	...
	for( i=0; i<30; i++ )
    {
		...
		if( KD01Data[i].SAMPLE_P == -1 )
		{
			...
			if ( Trim(KD01Data[i].MEMO) != "" ) {
				//pPnlP[i]->Hint = KD01Data[i].MEMO;
				pEdt1[i]->Hint = KD01Data[i].MEMO;
				pEdt2[i]->Hint = KD01Data[i].MEMO;
				pEdt3[i]->Hint = KD01Data[i].MEMO;
				pEdt4[i]->Hint = KD01Data[i].MEMO;
				pScrl[i]->Hint = KD01Data[i].MEMO;
				pPnlN[i]->Hint = KD01Data[i].MEMO;
