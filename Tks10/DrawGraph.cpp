...
*  create        : 1999.11.20                                          *
*  update        :                                                     *
*  contents      : ﾘｰﾄﾞ線長さ寸法測定測定履歴、工程能力作画関数        *
...
void DispKoutei( int iSp )
{
   ...
   //表示データを求める
   int iLinkSp;
   if(  KD01Data[iSp-1].LINK == 0 ){
       iLinkSp = iSp ;
   }else {
       iLinkSp = KD01Data[iSp-1].LINK;
   }
   ...
   for( i=iLotStart; i < iRtn ; i++ ) {
          ...
          if( KD03WData[iLinkSp-1].SOKUTEI_NM[i] == 0 ) continue ;

           //履歴データ個数
           n += KD03WData[iLinkSp-1].SOKUTEI_NM[i] ;

           // 平均値の計算
           dAve += KD03WData[iLinkSp-1].SOKUTE_AVE[i]*KD03WData[iLinkSp-1].SOKUTEI_NM[i];

           for( j=0; j< KD03WData[iLinkSp-1].SOKUTEI_NM[i]; j++ ) {

                // 最大値、最小値
                if( KD03WData[iLinkSp-1].SOKUTEI[i][j] > dMax ) dMax = KD03WData[iLinkSp-1].SOKUTEI[i][j];
                if( KD03WData[iLinkSp-1].SOKUTEI[i][j] < dMin ) dMin = KD03WData[iLinkSp-1].SOKUTEI[i][j];

                //不合格ﾃﾞｰﾀのﾁｪｯｸ
                if(  KD03WData[iLinkSp-1].SOKUTEI[i][j] >  KD03WData[iLinkSp-1].LD_LNG_MAX[i] ||
                     KD03WData[iLinkSp-1].SOKUTEI[i][j] <  KD03WData[iLinkSp-1].LD_LNG_MIN[i] )
                 FuryoCnt +=1 ;
   ...
   //ｶﾚﾝﾄ計測ﾃﾞｰﾀ
   if( KD01Data[iSp-1].SOKUTEI_NM_CUR != 0 )
   {
            //データ個数
            n += KD01Data[iSp-1].SOKUTEI_NM_CUR ;

            // 平均値の計算
            for( j=0; j< KD01Data[iSp-1].SOKUTEI_NM_CUR; j++ ) {
                dAve += KD01Data[iSp-1].SOKUTEICHI[j];

                //最大値、最小値
               if( KD01Data[iSp-1].SOKUTEICHI[j] > dMax ) dMax = KD01Data[iSp-1].SOKUTEICHI[j];
               if( KD01Data[iSp-1].SOKUTEICHI[j] < dMin ) dMin = KD01Data[iSp-1].SOKUTEICHI[j];

               //不合格ﾃﾞｰﾀのﾁｪｯｸ
                if(  KD01Data[iSp-1].SOKUTEICHI[j] >  KD01Data[iSp-1].LD_LNG_MAX ||
                     KD01Data[iSp-1].SOKUTEICHI[j] <  KD01Data[iSp-1].LD_LNG_MIN )
                 FuryoCnt +=1 ;
           }

   }

   // 計測ﾃﾞｰﾀがない場合、工程能力表示画面をクリア
   if( n== 0 ) {
   ...
   if( n!= 1 ){
       ...
       for( i=iLotStart; i<iRtn ; i++ )
       {
           for( int j=0; j< KD03WData[iLinkSp-1].SOKUTEI_NM[i]; j++ )
                dSigma += (KD03WData[iLinkSp-1].SOKUTEI[i][j] - dAve)*(KD03WData[iLinkSp-1].SOKUTEI[i][j] - dAve) ;
       }

       if( KD01Data[iSp-1].SOKUTEI_NM_CUR != 0 ) {
           for( i=0; i<KD01Data[iSp-1].SOKUTEI_NM_CUR ; i++ )
                dSigma += (KD01Data[iSp-1].SOKUTEICHI[i] - dAve)*(KD01Data[iSp-1].SOKUTEICHI[i] - dAve) ;
       }
   ...
   // 規格中心、規格幅
   double dA = KD01Data[iSp-1].LD_LNG_MAX + KD01Data[iSp-1].LD_LNG_MIN ;
   double dB = KD01Data[iSp-1].LD_LNG_MAX - KD01Data[iSp-1].LD_LNG_MIN ;
...
void SetRirekiData(int iSp)
{
  ...
  // 作画用変数の初期化
   ...
   for( i=iLotStart; i < iRtn ; i++ ) {
          ...
          if( KD03WData[iLinkSp-1].SOKUTEI_NM[i] != 0 ) {

              // 最大値、最小値、平均値の計算
              Max[i-iLotStart] = maxa;
              Min[i-iLotStart] = mina ;
              for(  j=0; j<KD03WData[iLinkSp-1].SOKUTEI_NM[i] ; j++ ){
                    if( KD03WData[iLinkSp-1].SOKUTEI[i][j] > Max[i-iLotStart] ) Max[i-iLotStart] = KD03WData[iLinkSp-1].SOKUTEI[i][j];
                    if( KD03WData[iLinkSp-1].SOKUTEI[i][j] < Min[i-iLotStart] ) Min[i-iLotStart] = KD03WData[iLinkSp-1].SOKUTEI[i][j];

                    Val[i-iLotStart] += KD03WData[iLinkSp-1].SOKUTEI[i][j] ;
              }

              Val[i-iLotStart] = Val[i-iLotStart]/KD03WData[iLinkSp-1].SOKUTEI_NM[i] ;
          }

          // ﾛｯﾄ番号
          str = KD03WData[iLinkSp-1].LOTNO[i] ;
   ...
   if( KD01Data[iSp-1].SOKUTEI_NM_CUR != 0 )
   {
         ...
         Min[iRRR] = mina ;
         for( i=0; i< KD01Data[iSp-1].SOKUTEI_NM_CUR ; i++ ){
              if( KD01Data[iSp-1].SOKUTEICHI[i] > Max[iRRR] ) Max[iRRR] = KD01Data[iSp-1].SOKUTEICHI[i];
              if( KD01Data[iSp-1].SOKUTEICHI[i] < Min[iRRR] ) Min[iRRR] = KD01Data[iSp-1].SOKUTEICHI[i];
         }

         // 平均値の計算
         for( i=0; i< KD01Data[iSp-1].SOKUTEI_NM_CUR; i++ )
              Val[iRRR] += KD01Data[iSp-1].SOKUTEICHI[i] ;
         Val[iRRR] = Val[iRRR]/KD01Data[iSp-1].SOKUTEI_NM_CUR ;
   }

   str = KD01Data[iSp-1].LOTNO ;
   if( str.Length() > 5 ) str = str.Delete(1,1);
...
void DrawRireki(TImage *GraphName,int n, char *Pstr, double Max[10], double Min[10], double Val[10], int iCur)
{
    ...
    //最大値、最小値の検索
    MaxY = KD01Data[SamplePositionIdx-1].LD_LNG_MAX ;
    MinY = KD01Data[SamplePositionIdx-1].LD_LNG_MIN;
...
*  create        : 1999.11                                             *
*  update        :                                                     *
*  contents      : KD03Wから読み込んだﾃﾞｰﾀ中にに指定したロットNo       *
*                :  有るかどうか のチェック                            *
*  written by    : 呉 亜娟(NKK unics Corp.)                            *
***********************************************************************/
int  CheckLotNo(int iSp)
{
   ...
   for( int i=0; i<  KD03WData[iLinkSp-1].LOTCNT; i++ ){
      if(  KD01Data[iSp-1].LOTNO ==  KD03WData[iLinkSp-1].LOTNO[i] )
      {
        iFlag = i ;
        break ;
       }
