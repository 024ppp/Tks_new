...
extern struct structKD91{
      ...
      double ZAKUT_MIN ;      //座屈 規格下
      double ZAKUT_MAX ;      //座屈 規格上
      int SOKUTEI_NM ;         //座屈 測定個数
      int SOKUTEI_SU ;         //座屈 測定個数
...
//座屈測定履歴作画用テーブル変数 KD93W
extern struct structKD93W{
      AnsiString DTKSHIN;         //得意先品番
      AnsiString LOTNO[50];        //ロットNO
      double ZAKUT_MIN[50] ;      //座屈 規格下
      double ZAKUT_MAX[50] ;      //座屈 規格上
...
extern int CurDataChg;            // 更新した場合 =1   更新しない場合 =0
...
extern void AddKd93W(void);
extern int  CheckLotNo(int ) ;
extern void CopyKD91Data(void) ;

extern int  IsNullSpace(AnsiString);
extern int  Kd91_Serch01(void);
extern int  Kd91_Serch02(void);
extern void Kd91_Update01(void);

extern long Kd93W_Create01(AnsiString, int, int) ;
extern int  Kd93W_Serch01();
