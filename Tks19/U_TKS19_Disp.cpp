...
*  file   name   : U_TKS19_DISP.cpp                                    *
*  create        : 1999.11                                             *
*  update        :                                                     *
*  contents      : 座屈測定GUI処理用関数                               *
    ...
    // ﾃﾞｰﾀ２にFocusがある
     if( pEdt2[SamplePositionIdx-1]->Focused()){
        ...
        if( KD91Data[SamplePositionIdx-1].SOKUTEI_NM > 2 )
        {
           ...
           while( 1 ){
               irec +=1 ;
               if( KD91Data[SamplePositionIdx-1+irec].SOKUTE_KBN  == 1){
               iFlg = 1;
                break ;}
           }
           if( iFlg == 0 ){
    ...
    if( pEdt3[SamplePositionIdx-1]->Focused() ||
        ...
        pScrl[SamplePositionIdx-1]->Focused() )   {
         ...
         if( CheckDtNo() < 0 ) return ;
          ...
          if( n >= KD91Data[SamplePositionIdx-1].SOKUTEI_NM  ){
               ...
               if( pEdt3[iLastPosition-1]->Focused() ||
                ...
                ToDrawGraph(SamplePositionIdx, iOption);
                 ...
                 //データの更新
                 pEdt3[SamplePositionIdx-1]->Text = KD91Data[SamplePositionIdx-1].SOKUTEICHI[n] ;
...
int iFlg, n;
   ...
   if(  Form1->BtnF01->Focused() ||
        ...
        if( KD91Data[iLastPosition-1].SOKUTEI_NM > 2 ){
   ...
   if( pEdt1[SamplePositionIdx-1]->Focused()){
        ...
        if( iFlg == 0 ){
              ...
              Form1->edttlno->SetFocus();
               ...
               iSaveFlag = 1;
              // 計測データ数 ＞２の場合
              if( KD91Data[SamplePositionIdx-irec-1].SOKUTEI_NM > 2 ){
         ...
         if( CheckDtNo() < 0 ) return ;
          ...
          if( n <= 3){
                ...
                pEdt2[SamplePositionIdx-1]->SetFocus();
                 ...
                 // データの更新
                 pEdt3[SamplePositionIdx-1]->Text = KD91Data[SamplePositionIdx-1].SOKUTEICHI[n-1-1] ;
...
void CheckInKikaku(int iBeep)
{
  ...
  int     errflg=0;;
    ...
    if( SamplePositionIdx > 0 && (SamplePositionIdx >= iFirstPosition &&
       ...
       if( ( dBuf > KD91Data[SamplePositionIdx-1].ZAKUT_MAX  ||
            dBuf < KD91Data[SamplePositionIdx-1].ZAKUT_MIN ) &&
            KD91Data[SamplePositionIdx-1].SOKUTE_KBN == 1 &&
             DataNoIdx <= KD91Data[SamplePositionIdx-1].SOKUTEI_NM_CUR ){
	...
	RSInputFlag = 0;
      ...
      if( KD91Data[SamplePositionIdx-1].SOKUTEI_NM > 2 && DataNoIdx >=3 && errflg != 3){

        int k=0;
        for(int  i=2; i< KD91Data[SamplePositionIdx-1].SOKUTEI_NM; i++ ){

             if( ( KD91Data[SamplePositionIdx-1].SOKUTEICHI[i] > KD91Data[SamplePositionIdx-1].ZAKUT_MAX  ||
                  KD91Data[SamplePositionIdx-1].SOKUTEICHI[i] < KD91Data[SamplePositionIdx-1].ZAKUT_MIN ) &&
                  KD91Data[SamplePositionIdx-1].SOKUTE_KBN == 1 &&
                  DataNoIdx <= KD91Data[SamplePositionIdx-1].SOKUTEI_NM_CUR ){
...
{
   if( KD91Data[SamplePositionIdx-1].SOKUTEI_NM > 2  &&
       KD91Data[SamplePositionIdx-1].SOKUTE_KBN == 1 &&  DataNoIdx >  2 )
   {
       int n = pEdt4[SamplePositionIdx-1]->Value;

       if( fabs( pEdt3[SamplePositionIdx-1]->Value - KD91Data[SamplePositionIdx-1].SOKUTEICHI[n-1]) > 0.000001 ){
           ...
           // 計測したデータ数をｶﾝトー
          if ( KD91Data[SamplePositionIdx-1].SOKUTEI_NM_CUR < n )
               KD91Data[SamplePositionIdx-1].SOKUTEI_NM_CUR = n ;
...
void DisplayInfo(void)
{
     ...
     Form1->Pnl5->Caption = FormatFloat("0", KD91Data[SamplePositionIdx-1].ZAKUT_MIN) ;
     Form1->Pnl6->Caption = FormatFloat("0", KD91Data[SamplePositionIdx-1].ZAKUT_MAX) ;
     Form1->Pnl7->Caption = KD91Data[SamplePositionIdx-1].DTKSCOD ;
     Form1->Pnl8->Caption = KD91Data[SamplePositionIdx-1].DTKSHIN ;
     Form1->Pnl9->Caption = KD91Data[SamplePositionIdx-1].LOTNO ;
     Form1->Pnl10->Caption = KD91Data[SamplePositionIdx-1].HINBAN ;
     Form1->Pnl11->Caption = KD91Data[SamplePositionIdx-1].ZISCOD ;
     Form1->Pnl12->Caption = KD91Data[SamplePositionIdx-1].ZISNAM ;
...
void SetRed(int iFlg)
{
   ...
   for( i=0; i<30 ; i++ ){
      ...
      //最初登録のときに規格チェックしない
      if( iFlg == 1 && IsNullSpace( KD91Data[i].ADDYMD )) {
          pEdt1[i]->Font->Color = clBlack;
          pEdt2[i]->Font->Color = clBlack;
          pEdt3[i]->Font->Color = clBlack;
          pEdt4[i]->Font->Color == clBlack;
      ...
      if( KD91Data[i].SOKUTE_KBN == 1 ){
           ...
           //測定値１
           if((KD91Data[i].SOKUTEICHI[0] >  KD91Data[i].ZAKUT_MAX ||
              KD91Data[i].SOKUTEICHI[0] <  KD91Data[i].ZAKUT_MIN ) &&
               KD91Data[i].SOKUTEI_NM_CUR>=1){
           ...
           //測定値2
           if( (KD91Data[i].SOKUTEICHI[1] >  KD91Data[i].ZAKUT_MAX ||
               KD91Data[i].SOKUTEICHI[1] <  KD91Data[i].ZAKUT_MIN ) &&
                KD91Data[i].SOKUTEI_NM_CUR>=2){
           ...
           if( KD91Data[i].SOKUTEI_NM > 2 ){
               ...
               k = 0;
               for( j=2; j< KD91Data[i].SOKUTEI_NM ; j++){

                     //測定値3～
                    if( (KD91Data[i].SOKUTEICHI[j] >  KD91Data[i].ZAKUT_MAX ||
                        KD91Data[i].SOKUTEICHI[j] <  KD91Data[i].ZAKUT_MIN ) &&
                         KD91Data[i].SOKUTEI_NM_CUR>=j){
                         k = 1 ;
                         break ;
                    }
