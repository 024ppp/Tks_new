...
//---------------------------------------------------------------------------
// KD51構造体変数
structKD51 KD51Data[30];      // 母体比重ﾃﾞｰﾀﾌｧｲﾙ  KD51

//KD53W構造体変数
structKD53W KD53WData[30];    // 母体比重履歴ﾃﾞｰﾀﾜｰｸﾌｧｲﾙ  KD53W


//---------------------------------------------------------------------------
//画面操作用変数
int iRec_Kd51;              // 一覧検索結果件数
...
AnsiString MSG_16 = "コンミの計測結果を反映させますか？" ;
...
//測定明細
TNumberEdit *pEdt1_S[4] ;    //  列ﾃﾞｰﾀ１のｺﾝﾄﾛｰﾙを保存
TNumberEdit *pEdt2_S[4] ;    //  列ﾃﾞｰﾀ2 のｺﾝﾄﾛｰﾙを保存
TNumberEdit *pEdt3_S[4] ;    //  列ﾃﾞｰﾀ3 のｺﾝﾄﾛｰﾙを保存
...
void __fastcall TForm1::OnNotifyWriteData(TMessage &Message)
{
 ...
 // [Data]
    ...
    if( KikiType == 1 ){
       ...
       //20160728 電子天秤の切替機接続に対応
       /*
        // チャネル番号１～５まで、重量を計測しない
       if( iKomoku == 4 && iChNo != 6 ) return ;
       */
       if( iKojoKbn == 0 ){
           //本社の場合、従来通りに処理を行う
...
//      ToDrawGraph(1, iOption);
   ...
   if( HaveSokutei ){
      ...
      }else if( KD51Data[SamplePositionIdx-1].KEIJYO_KBN == 5 ){ //コンミの場合 2002/11/02 E.Takase
         Rad1->Caption ="   内  径" ;
         Rad2->Caption ="　 外  径" ;
         Rad3->Caption ="   厚  さ" ;
         Rad4->Caption ="   重  量" ;
