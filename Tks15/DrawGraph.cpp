...
void DispKoutei( int iSp )
{
   ...
   //表示データを求める
   int iLinkSp;
   if(  KD51Data[iSp-1].LINK == 0 ){
       iLinkSp = iSp ;
   }else {
       iLinkSp = KD51Data[iSp-1].LINK;
   }
   ...
   for( i=iLotStart; i < iRtn ; i++ ){
          ...
          if(KD53WData[iLinkSp-1].SOKUTEI_NM[i] == 0 ) continue ;

           //履歴データ個数
           n += KD53WData[iLinkSp-1].SOKUTEI_NM[i] ;

           // 平均値の計算
           dAve += KD53WData[iLinkSp-1].SOKUTE_AVE[i]*KD53WData[iLinkSp-1].SOKUTEI_NM[i];

           for( j=0; j< KD53WData[iLinkSp-1].SOKUTEI_NM[i]; j++ ) {

                if( KD53WData[iLinkSp-1].SOKUTEI[i][j] > dMax ) dMax = KD53WData[iLinkSp-1].SOKUTEI[i][j];
                if( KD53WData[iLinkSp-1].SOKUTEI[i][j] < dMin ) dMin = KD53WData[iLinkSp-1].SOKUTEI[i][j];

                 //不合格ﾃﾞｰﾀのﾁｪｯｸ
                if(  KD53WData[iLinkSp-1].SOKUTEI[i][j] >  KD53WData[iLinkSp-1].BOTAI_MAX[i] ||
                     KD53WData[iLinkSp-1].SOKUTEI[i][j] <  KD53WData[iLinkSp-1].BOTAI_MIN[i] )
                 FuryoCnt +=1 ;
   ...
   //ｶﾚﾝﾄｶｰｿﾙがある計測ﾃﾞｰﾀ
   if( KD51Data[iSp-1].SOKUTEI_NB_CUR != 0 )
   {

            //データ個数
            n += KD51Data[iSp-1].SOKUTEI_NB_CUR ;

            // 平均値の計算
            for( j=0; j< KD51Data[iSp-1].SOKUTEI_NB_CUR; j++ ) {
                dAve += KD51Data[iSp-1].SOKUTEI_B[j];

               if( KD51Data[iSp-1].SOKUTEI_B[j] > dMax ) dMax = KD51Data[iSp-1].SOKUTEI_B[j];
               if( KD51Data[iSp-1].SOKUTEI_B[j] < dMin ) dMin = KD51Data[iSp-1].SOKUTEI_B[j];

               //不合格ﾃﾞｰﾀのﾁｪｯｸ
                if(  KD51Data[iSp-1].SOKUTEI_B[j] >  KD51Data[iSp-1].BOTAI_MAX ||
                     KD51Data[iSp-1].SOKUTEI_B[j] <  KD51Data[iSp-1].BOTAI_MIN )
                 FuryoCnt +=1 ;
   ...
   if( n!= 1 ){
       ...
       for( i=iLotStart; i<iRtn ; i++ )
       {
           for( int j=0; j< KD53WData[iLinkSp-1].SOKUTEI_NM[i]; j++ )
                dSigma += (KD53WData[iLinkSp-1].SOKUTEI[i][j] - dAve)*(KD53WData[iLinkSp-1].SOKUTEI[i][j] - dAve) ;
       }

       if( KD51Data[iSp-1].SOKUTEI_NB_CUR != 0 ) {
           for( i=0; i<KD51Data[iSp-1].SOKUTEI_NB_CUR ; i++ )
                dSigma += (KD51Data[iSp-1].SOKUTEI_B[i] - dAve)*(KD51Data[iSp-1].SOKUTEI_B[i] - dAve) ;
       }

       dSigma = sqrt(dSigma/(n-1)) ;
   }

   double dA = KD51Data[iSp-1].BOTAI_MAX + KD51Data[iSp-1].BOTAI_MIN ;
   double dB = KD51Data[iSp-1].BOTAI_MAX - KD51Data[iSp-1].BOTAI_MIN ;
...
void SetRirekiData(int iSp)
{
   ...
   for( i=iLotStart; i < iRtn ; i++ ) {
          ...
          if( KD53WData[iLinkSp-1].SOKUTEI_NM[i] != 0 ) {

              // 最大値、最小値、平均値の計算
              Max[i-iLotStart] = maxa;
              Min[i-iLotStart] = mina ;
              for(  j=0; j<KD53WData[iLinkSp-1].SOKUTEI_NM[i] ; j++ ){
                    if( KD53WData[iLinkSp-1].SOKUTEI[i][j] > Max[i-iLotStart] ) Max[i-iLotStart] = KD53WData[iLinkSp-1].SOKUTEI[i][j];
                    if( KD53WData[iLinkSp-1].SOKUTEI[i][j] < Min[i-iLotStart] ) Min[i-iLotStart] = KD53WData[iLinkSp-1].SOKUTEI[i][j];

                    Val[i-iLotStart] += KD53WData[iLinkSp-1].SOKUTEI[i][j] ;
              }

              Val[i-iLotStart] = Val[i-iLotStart]/KD53WData[iLinkSp-1].SOKUTEI_NM[i] ;
          }

          str = KD53WData[iLinkSp-1].LOTNO[i] ;
   ...
   if( KD51Data[iSp-1].SOKUTEI_NB_CUR != 0 )
   {
         ...
         Min[iRRR] = mina ;
         for( i=0; i< KD51Data[iSp-1].SOKUTEI_NB_CUR ; i++ ){
              if( KD51Data[iSp-1].SOKUTEI_B[i] > Max[iRRR] ) Max[iRRR] = KD51Data[iSp-1].SOKUTEI_B[i];
              if( KD51Data[iSp-1].SOKUTEI_B[i] < Min[iRRR] ) Min[iRRR] = KD51Data[iSp-1].SOKUTEI_B[i];
         }

         // 平均値の計算
         for( i=0; i< KD51Data[iSp-1].SOKUTEI_NB_CUR; i++ )
              Val[iRRR] += KD51Data[iSp-1].SOKUTEI_B[i] ;
         Val[iRRR] = Val[iRRR]/KD51Data[iSp-1].SOKUTEI_NB_CUR ;
   }

   str = KD51Data[iSp-1].LOTNO ;
   if( str.Length() > 5 ) str = str.Delete(1,1);
...
void DrawRireki(TImage *GraphName,int n, char *Pstr, double Max[10], double Min[10], double Val[10], int iCur)
{
    ...
    //最大値、最小値の検索
    MaxY = KD51Data[SamplePositionIdx-1].BOTAI_MAX;
    MinY = KD51Data[SamplePositionIdx-1].BOTAI_MIN;
...
*  create        : 1999.11                                             *
*  update        :                                                     *
*  contents      : KD53Wから読み込んだﾃﾞｰﾀ中にに指定したロットNo       *
*                :  有るかどうか のチェック                            *
*  written by    : 呉 亜娟(NKK unics Corp.)                            *
***********************************************************************/
int  CheckLotNo(int iSp)
{
   ...
   for( int i=0; i<  KD53WData[iLinkSp-1].LOTCNT; i++ ){
      if(  KD51Data[iSp-1].LOTNO ==  KD53WData[iLinkSp-1].LOTNO[i] )
      {
        iFlag = i ;
        break ;
       }
