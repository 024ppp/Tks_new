...
extern struct structKD71{
      ...
      double KOYU_MAX ;        //固有抵抗率 規格上
      double KOYU_MIN ;        //固有抵抗率 規格下
      double KOYU_DEN;         //固有抵抗率 電流
      int    KOYU_PRO;         //固有抵抗率 指示プローブ
      int    SOKUTEI_NM ;      //測定個数
      int    SOKUTEI_SU ;      //測定個数
      double SOKUTE_AVE ;      //測定平均値
      double SOKUTEI_T[100] ;  //測定値     固有抵抗率
      double SOKUTEI_S[100] ;  //測定値     厚さ寸法
      double SOKUTEI_H[100] ;  //測定値     幅寸法
      double SOKUTEV_R[100] ;  //測定値     電圧 右
      double SOKUTEV_L[100] ;  //測定値     電圧 左
      double SOKUTEA_R[100] ;  //測定値     電流 右
      double SOKUTEA_L[100] ;  //測定値     電流 左
      int    SOKUTEI_P[100] ;  //測定値     プローブ
      ...
      int SOKUTEI_NT_CUR;      //ｶﾚﾝﾄ測定データNO
      int SOKUTEI_NS_CUR;      //ｶﾚﾝﾄ測定データNO
      int SOKUTEI_NH_CUR;      //ｶﾚﾝﾄ測定データNO
      int SOKUTEI_NVL_CUR;     //ｶﾚﾝﾄ測定データNO
      int SOKUTEI_NAL_CUR;     //ｶﾚﾝﾄ測定データNO
      int SOKUTEI_NVR_CUR;     //ｶﾚﾝﾄ測定データNO
      int SOKUTEI_NAR_CUR;     //ｶﾚﾝﾄ測定データNO
...
//固有抵抗率履歴作画用テーブル変数 KD73W
extern struct structKD73W{
      AnsiString DTKSHIN;          //得意先品番
      AnsiString LOTNO[50];        //ロットNO
      double KOYU_MAX[50] ;        //固有抵抗率 規格上
      double KOYU_MIN[50] ;        //固有抵抗率 規格下
      double SOKUTEI[50][100] ;    //測定値 抵抗
      double SOKUTEV_R[50][100] ;  //測定値     電圧 右  2002/11/29 追加 E.Takase
      double SOKUTEV_L[50][100] ;  //測定値     電圧 左  2002/11/29 追加 E.Takase
      double SOKUTE_AVE[50] ;      //測定平均値 抵抗
      double SOKUTE_AVE_R[50] ;    //測定平均値 電圧 右  2002/11/29 追加 E.Takase
      double SOKUTE_AVE_L[50] ;    //測定平均値 電圧 左  2002/11/29 追加 E.Takase
      int    KEIJYO_KBN ;          //形状区分            2002/11/29 追加 E.Takase
      int LOTCNT;
      int SOKUTEI_NM[50];
}KD73WData[30];
...
//測定明細
extern TNumberEdit *pEdt1_S[2] ;    //  列ﾃﾞｰﾀ１のｺﾝﾄﾛｰﾙを保存
extern TNumberEdit *pEdt2_S[2] ;    //  列ﾃﾞｰﾀ2 のｺﾝﾄﾛｰﾙを保存
extern TNumberEdit *pEdt3_S[2] ;    //  列ﾃﾞｰﾀ3 のｺﾝﾄﾛｰﾙを保存

extern TNumberEdit *pEdt1_V[2] ;    //  列ﾃﾞｰﾀ１のｺﾝﾄﾛｰﾙを保存
extern TNumberEdit *pEdt2_V[2] ;    //  列ﾃﾞｰﾀ2 のｺﾝﾄﾛｰﾙを保存
extern TNumberEdit *pEdt3_V[2] ;    //  列ﾃﾞｰﾀ3 のｺﾝﾄﾛｰﾙを保存
extern TNumberEdit *pEdt1_A[2] ;    //  列ﾃﾞｰﾀ１のｺﾝﾄﾛｰﾙを保存
extern TNumberEdit *pEdt2_A[2] ;    //  列ﾃﾞｰﾀ2 のｺﾝﾄﾛｰﾙを保存
extern TNumberEdit *pEdt3_A[2] ;    //  列ﾃﾞｰﾀ3 のｺﾝﾄﾛｰﾙを保存
...
extern int AllDataChg;             // 画面に表示しているﾃﾞｰﾀ更新されたかどうか
extern int CurSampChg;             // ｶﾚﾝﾄｻﾝﾌﾟﾙﾎﾟｼﾞｼｮﾝNOのﾃﾞｰﾀを更新されたかどうか
extern int CurDataChg;             // ｶﾚﾝﾄ計測値を更新されたかどうか
