...
void WriteLog2( AnsiString Msg);		// 2004/08/30 A.Tamura ログ出力
char tempo[1024];
...
//        sprintf(tempo, "Wirte: SId = %d, FId = %d, Len = %d, Buf = %s", SId, FId, Len, WrtBuf);
//        WriteLog(tempo);
...
//        sprintf(tempo, "Read: SId = %d, Buf = %s", SId, ReadBuf);
//        WriteLog(tempo);
...
void WriteLog(char *Msg)
{
	...
	if ((Logfn = fopen(FileName, "a+")) == NULL) {
		Logfn = fopen(FileName, "w+");
		if (Logfn == NULL) {
			//MessageBox(GetFocus(), "Create Log Data file error", "PC12Ctrl.log", MB_OK);
			return;
		}
	}

	fprintf(Logfn, "%04d/%02d/%02d  %02d:%02d:%02d  %s\n", (int)T_Time.wYear, (int)T_Time.wMonth, (int)T_Time.wDay,
					(int)T_Time.wHour, (int)T_Time.wMinute, (int)T_Time.wSecond, Msg);

	fclose(Logfn);
}

// 2004/08/30 A.Tamura ＤＢ保存処理のログ出力
void WriteLog2(AnsiString Msg)
{
    static int Ini_Flag = 0;
    static char FileName[256];
    ...
    if (Ini_Flag == 0) {
        ...
        GetCurrentDirectory(sizeof(DirName), DirName);
	    sprintf(FileName, "%s\\DBError.Log", DirName);
        Ini_Flag = 1;
	...
	fprintf(Logfn, "%04d/%02d/%02d  %02d:%02d:%02d  %s\n", (int)T_Time.wYear, (int)T_Time.wMonth, (int)T_Time.wDay,
					(int)T_Time.wHour, (int)T_Time.wMinute, (int)T_Time.wSecond, Msg.c_str());
