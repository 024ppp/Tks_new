...
void DispKoutei( int iSp )
{
   ...
   //表示データを求める
   int iLinkSp;
   if(  KD81Data[iSp-1].LINK == 0 ){
       iLinkSp = iSp ;
   }else {
       iLinkSp = KD81Data[iSp-1].LINK;
   }
   ...
   for( i=iLotStart; i < iRtn ; i++ ){
          ...
          if(KD83WData[iLinkSp-1].SOKUTEI_NM[i] == 0 ) continue ;

           //履歴データ個数
           n += KD83WData[iLinkSp-1].SOKUTEI_NM[i] ;

           // 平均値の計算
           dAve += KD83WData[iLinkSp-1].SOKUTE_AVE[i]*KD83WData[iLinkSp-1].SOKUTEI_NM[i];

           for( j=0; j< KD83WData[iLinkSp-1].SOKUTEI_NM[i]; j++ ) {

                if( KD83WData[iLinkSp-1].SOKUTEI[i][j] > dMax ) dMax = KD83WData[iLinkSp-1].SOKUTEI[i][j];
                if( KD83WData[iLinkSp-1].SOKUTEI[i][j] < dMin ) dMin = KD83WData[iLinkSp-1].SOKUTEI[i][j];

                //不合格ﾃﾞｰﾀのﾁｪｯｸ
                if(  KD83WData[iLinkSp-1].SOKUTEI[i][j] >  KD83WData[iLinkSp-1].MAGE_MAX[i] ||
                     KD83WData[iLinkSp-1].SOKUTEI[i][j] <  KD83WData[iLinkSp-1].MAGE_MIN[i] )
                 FuryoCnt +=1 ;
           }
   }

   //ｶﾚﾝﾄｶｰｿﾙがある計測ﾃﾞｰﾀ
   if( KD81Data[iSp-1].SOKUTEI_NM_CUR != 0 )
   {

            //データ個数
            n += KD81Data[iSp-1].SOKUTEI_NM_CUR ;

            // 平均値の計算
            for( j=0; j< KD81Data[iSp-1].SOKUTEI_NM_CUR; j++ ) {
                dAve += KD81Data[iSp-1].SOKUTEI_M[j];

               if( KD81Data[iSp-1].SOKUTEI_M[j] > dMax ) dMax = KD81Data[iSp-1].SOKUTEI_M[j];
               if( KD81Data[iSp-1].SOKUTEI_M[j] < dMin ) dMin = KD81Data[iSp-1].SOKUTEI_M[j];

               //不合格ﾃﾞｰﾀのﾁｪｯｸ
                if(  KD81Data[iSp-1].SOKUTEI_M[j] >  KD81Data[iSp-1].MAGE_MAX ||
                     KD81Data[iSp-1].SOKUTEI_M[j] <  KD81Data[iSp-1].MAGE_MIN )
                 FuryoCnt +=1 ;
           }
   ...
   if( n!= 1 ){
       ...
       for( i=iLotStart; i<iRtn ; i++ )
       {
           for( int j=0; j< KD83WData[iLinkSp-1].SOKUTEI_NM[i]; j++ )
                dSigma += (KD83WData[iLinkSp-1].SOKUTEI[i][j] - dAve)*(KD83WData[iLinkSp-1].SOKUTEI[i][j] - dAve) ;
       }

       if( KD81Data[iSp-1].SOKUTEI_NM_CUR != 0 ) {
           for( i=0; i<KD81Data[iSp-1].SOKUTEI_NM_CUR ; i++ )
                dSigma += (KD81Data[iSp-1].SOKUTEI_M[i] - dAve)*(KD81Data[iSp-1].SOKUTEI_M[i] - dAve) ;
       }
   ...
   //K
   double dA = KD81Data[iSp-1].MAGE_MAX + KD81Data[iSp-1].MAGE_MIN ;
   double dB = KD81Data[iSp-1].MAGE_MAX - KD81Data[iSp-1].MAGE_MIN ;
...
void SetRirekiData(int iSp)
{
  ...
  // 作画用変数の初期化
   ...
   for( i=iLotStart; i < iRtn ; i++ ) {
          ...
          if( KD83WData[iLinkSp-1].SOKUTEI_NM[i] != 0 ) {

              // 最大値、最小値、平均値の計算
              Max[i-iLotStart] = maxa;
              Min[i-iLotStart] = mina ;
              for(  j=0; j<KD83WData[iLinkSp-1].SOKUTEI_NM[i] ; j++ ){
                    if( KD83WData[iLinkSp-1].SOKUTEI[i][j] > Max[i-iLotStart] ) Max[i-iLotStart] = KD83WData[iLinkSp-1].SOKUTEI[i][j];
                    if( KD83WData[iLinkSp-1].SOKUTEI[i][j] < Min[i-iLotStart] ) Min[i-iLotStart] = KD83WData[iLinkSp-1].SOKUTEI[i][j];

                    Val[i-iLotStart] += KD83WData[iLinkSp-1].SOKUTEI[i][j] ;
              }

              Val[i-iLotStart] = Val[i-iLotStart]/KD83WData[iLinkSp-1].SOKUTEI_NM[i] ;
          }

          str = KD83WData[iLinkSp-1].LOTNO[i] ;
   ...
   if( KD81Data[iSp-1].SOKUTEI_NM_CUR != 0 )
   {
         ...
         Min[iRRR] = mina ;
         for( i=0; i< KD81Data[iSp-1].SOKUTEI_NM_CUR ; i++ ){
              if( KD81Data[iSp-1].SOKUTEI_M[i] > Max[iRRR] ) Max[iRRR] = KD81Data[iSp-1].SOKUTEI_M[i];
              if( KD81Data[iSp-1].SOKUTEI_M[i] < Min[iRRR] ) Min[iRRR] = KD81Data[iSp-1].SOKUTEI_M[i];
         }

         // 平均値の計算
         for( i=0; i< KD81Data[iSp-1].SOKUTEI_NM_CUR; i++ )
              Val[iRRR] += KD81Data[iSp-1].SOKUTEI_M[i] ;
         Val[iRRR] = Val[iRRR]/KD81Data[iSp-1].SOKUTEI_NM_CUR ;
   }

   str = KD81Data[iSp-1].LOTNO ;
   if( str.Length() > 5 ) str = str.Delete(1,1);
...
void DrawRireki(TImage *GraphName,int n, char *Pstr, double Max[10], double Min[10], double Val[10], int iCur)
{
    ...
    //最大値、最小値の検索
    MaxY = KD81Data[SamplePositionIdx-1].MAGE_MAX ;
    MinY = KD81Data[SamplePositionIdx-1].MAGE_MIN;
...
*  create        : 1999.11                                             *
*  update        :                                                     *
*  contents      : KD83Wから読み込んだﾃﾞｰﾀ中にに指定したロットNo       *
*                :  有るかどうか のチェック                            *
*  written by    : 呉 亜娟(NKK unics Corp.)                            *
***********************************************************************/
int  CheckLotNo(int iSp)
{
   ...
   for( int i=0; i<  KD83WData[iLinkSp-1].LOTCNT; i++ ){
      if(  KD81Data[iSp-1].LOTNO ==  KD83WData[iLinkSp-1].LOTNO[i] )
      {
        iFlag = i ;
        break ;
       }
