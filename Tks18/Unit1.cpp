...
//---------------------------------------------------------------------------
// KD81構造体変数
structKD81 KD81Data[30];      // 抗折力ﾃﾞｰﾀﾌｧｲﾙ  KD81

//KD83W構造体変数
structKD83W KD83WData[30];    // 抗折力履歴ﾃﾞｰﾀﾜｰｸﾌｧｲﾙ  KD83W

//KD51構造体変数
structKD51 KD51Data[30];      // 母体比重ﾃﾞｰﾀﾜｰｸﾌｧｲﾙ  KD51
//---------------------------------------------------------------------------
//画面操作用変数
int iRec_Kd81;              // 一覧検索結果件数
...
long Kd83W_Create01(AnsiString, int, int) ;
int  Kd83W_Serch01();
     ...
     if (iShareFlag == 2 && iRSDataFlag == 1 ) {
        ...
        if( nNumber > 0 &&  strncmp( SharedBuff, "OK", 2) != 0 &&
           ...
           if( DataNoIdx == 1 ) {
             ...
             //2003.02.08 E.Takase
             if ( KD81Data[SamplePositionIdx-1].KEIJYO_KBN == 5 ) { //コンミ
                  pEdt1_S[iKomoku-1]->Text = FormatFloat("#0.00",dVal );
             } else {
...
void ExecF4(void)
{
   ...
   //厚さと幅を計測の場合
   if( iKomoku == 3 ){
      if( KD81Data[SamplePositionIdx-1].KEIJYO_KBN == 4 ){
           iKomoku = 2 ;
      ...
      if( KD81Data[SamplePositionIdx-1].KEIJYO_KBN == 5 ){
          iKomoku =3 ;
...
void __fastcall TForm1::Rad1Click(TObject *Sender)
{
   ...
   for( i=0; i<30; i++ ){
        if( KD81Data[i].SOKUTE_KBN == 1 &&
            KD81Data[i].SOKUTEI_NM > 2 ){

            n = pEdt4[i]->Value ;
            if( n > 3 ){
               pScrl[i]->Position = 3 ;
               pEdt4[i]->Text="3";
               pEdt3[i]->Text = FormatFloat("#0.0", KD81Data[i].SOKUTEI_M[2]);
            }
        }
...
void __fastcall TForm1::BtnF07Click(TObject *Sender)
{
   ...
   iKskStart = 0 ;
	...
	if( CheckBoxRireki->Checked ){
	     ...
	     //KD83W履歴ﾃﾞｰﾀテーブルの作成
	     //int iRec = Kd83W_Create01(KD81Data[0].KENSA_YMD, KD81Data[0].TOLEY_NO, LOTMAX);
	     int iRec = Kd83W_Create01(strVal01, strVal02.ToIntDef(0), LOTMAX);
	     if( iRec >0 ){

	        //KD83W履歴ﾃﾞｰﾀﾌｧｲﾙを読み込む
	        i  =  Kd83W_Serch01();
...
//        ToDrawGraph(1, iOption);
    ...
    for( i=0; i<30; i++ )
    {
          ...
          //2003.02.08 E.Takase
          if ( KD81Data[i].KEIJYO_KBN == 5 ) { //コンミ
             pEdt1[i]->FPos = 2;
             pEdt2[i]->FPos = 2;
             pEdt3[i]->FPos = 2;
          } else {
          ...
          if( KD81Data[i].SAMPLE_P == -1 )
          {
             ...
             */
               ...
               //2003.02.08 E.Takase
               if ( KD81Data[i].KEIJYO_KBN == 5 ) { //コンミ
                  pEdt1[i]->Text =  FormatFloat("#0.00",KD81Data[i].SOKUTEI_M[0] );
                  pEdt2[i]->Text =  FormatFloat("#0.00",KD81Data[i].SOKUTEI_M[1] );

               } else {
                  pEdt1[i]->Text =  FormatFloat("#0.0",KD81Data[i].SOKUTEI_M[0] );
                  pEdt2[i]->Text =  FormatFloat("#0.0",KD81Data[i].SOKUTEI_M[1] );
               }
               pPnlN[i]->Caption = IntToStr( KD81Data[i].SOKUTEI_NM) ;
               ...
               if( KD81Data[i].SOKUTEI_NM > 2 )
               {
                 //2003.02.08 E.Takase
                 if ( KD81Data[i].KEIJYO_KBN == 5 ) { //コンミ
                    pEdt3[i]->Text =  FormatFloat("#0.00",KD81Data[i].SOKUTEI_M[2] );
                 } else {
                    pEdt3[i]->Text =  FormatFloat("#0.0",KD81Data[i].SOKUTEI_M[2] );
                 }
                ...
                if(  KD81Data[i].SOKUTE_KBN == 1 ){
                  ...
                  if( KD81Data[i].SOKUTEI_NM > 2 )
                  {
                    ...
                    // ScrollBarの初期値設定
                    pScrl[i]->Min = 3;
                    pScrl[i]->Max = KD81Data[i].SOKUTEI_NM;
                    pScrl[i]->Position = 3 ;
                  ...
                  iLastPosition = KD81Data[i].SAMPLE_P ;
               }else{
                   // ﾃﾞｰﾀ１、ﾃﾞｰﾀ２ｺﾝﾄﾛｰﾙ
                 /*  IsEnabledEdt( pEdt1[i], 0 );
                   IsEnabledEdt( pEdt2[i], 0 );
    ...
    if( HaveSokutei ){
        ...
        }else if( KD81Data[SamplePositionIdx-1].KEIJYO_KBN == 5 ){ //コンミの場合 2002/11/02 E.Takase
           Rad2->Caption ="　  幅" ;
           if( iBotai){
              iKomoku = 3;
              Rad3->Checked = true ;
...
void __fastcall TForm1:: OnEditEnter( int spflag, int dtflag, TNumberEdit *pEdit )
{
 ...
 //   int SamplePositionIdx_old;
    ...
    //2003.02.08 E.Takase
    if ( KD81Data[spflag-1].KEIJYO_KBN == 5 ) { //コンミ
       pEdt1_S[2]->FPos = 2;
       pEdt2_S[2]->FPos = 2;
       pEdt3_S[2]->FPos = 2;
    } else {
    ...
    if( spflag != SamplePositionIdx ){
      ...
      //測定明細項目名の設定
        ...
        }else if( KD81Data[SamplePositionIdx-1].KEIJYO_KBN == 5 ){
           Rad2->Caption ="　  幅" ;
           if( iKomoku == 2 ) {
              Rad3->Checked = true ;
              iKomoku = 3 ;
    ...
    //測定データ明細の表示
    if( KD81Data[SamplePositionIdx-1].KEIJYO_KBN != 4 ){
         pEdt1_S[0]->Text =  FormatFloat("#0.000",KD81Data[SamplePositionIdx-1].SOKUTEI_S[0] );
         pEdt2_S[0]->Text =  FormatFloat("#0.000",KD81Data[SamplePositionIdx-1].SOKUTEI_S[1] );
     }else{
         pEdt1_S[0]->Text =  "0.000";
         pEdt2_S[0]->Text =  "0.000";
     }
     pEdt1_S[1]->Text =  FormatFloat("#0.000",KD81Data[SamplePositionIdx-1].SOKUTEI_H[0] );
     pEdt2_S[1]->Text =  FormatFloat("#0.000",KD81Data[SamplePositionIdx-1].SOKUTEI_H[1] );

     //2003.02.08 E.Takase
     if ( KD81Data[SamplePositionIdx-1].KEIJYO_KBN == 5 ) { //コンミ
        pEdt1_S[2]->Text =  FormatFloat("#0.00",KD81Data[SamplePositionIdx-1].SOKUTEI_K[0] );
        pEdt2_S[2]->Text =  FormatFloat("#0.00",KD81Data[SamplePositionIdx-1].SOKUTEI_K[1] );
     } else {
        pEdt1_S[2]->Text =  FormatFloat("#0.0",KD81Data[SamplePositionIdx-1].SOKUTEI_K[0] );
        pEdt2_S[2]->Text =  FormatFloat("#0.0",KD81Data[SamplePositionIdx-1].SOKUTEI_K[1] );
     }
     //スパン
     PnlSpan1->Caption  =  FormatFloat("#0.0",KD81Data[SamplePositionIdx-1].SOKUTEI_P[0] );
     PnlSpan2->Caption  =  FormatFloat("#0.0",KD81Data[SamplePositionIdx-1].SOKUTEI_P[1] );
    ...
    if( KD81Data[SamplePositionIdx-1].SOKUTEI_NM  > 2 ){
            ...
            for(  i=0; i<3; i++){
               //円柱の場合、飛ばす
               if( KD81Data[SamplePositionIdx-1].KEIJYO_KBN == 4 && i== 0  ) continue;
               pScrl_S[i]->Min = 0;
               pScrl_S[i]->Max = 99;

               // 抗折力に合わせて項目明細のScrollBarを設定する
               pScrl_S[i]->Min = 3;
               pScrl_S[i]->Max = KD81Data[SamplePositionIdx-1].SOKUTEI_NM;
               pScrl_S[i]->Position = pos ;
            }

            //測定値３～のﾃﾞｰﾀを画面へセット
            if( KD81Data[SamplePositionIdx-1].KEIJYO_KBN != 4 )
                pEdt3_S[0]->Text = FormatFloat("#0.000",KD81Data[SamplePositionIdx-1].SOKUTEI_S[pos-1] );
            else    // 円柱の場合
                pEdt3_S[0]->Text = "0.000";

             pEdt3_S[1]->Text = FormatFloat("#0.000",KD81Data[SamplePositionIdx-1].SOKUTEI_H[pos-1] );
             //2003.02.10 E.Takase
             if ( KD81Data[SamplePositionIdx-1].KEIJYO_KBN == 5 ) { //コンミ
                pEdt3_S[2]->Text = FormatFloat("#0.00",KD81Data[SamplePositionIdx-1].SOKUTEI_K[pos-1] );
             } else {
                pEdt3_S[2]->Text = FormatFloat("#0.0",KD81Data[SamplePositionIdx-1].SOKUTEI_K[pos-1] );
             }
             PnlSpan3->Caption  = FormatFloat("#0.0",KD81Data[SamplePositionIdx-1].SOKUTEI_P[pos-1] );

             for( i=0; i<3; i++ ){
                if(  KD81Data[SamplePositionIdx-1].KEIJYO_KBN == 4 && i== 0 ){
                     pEdt4_S[i]->Text = "0" ;
                     pPnl_S[i]->Caption = "0" ;
                }else {
                    ...
                    //測定個数の設定
                    pPnl_S[i]->Caption = IntToStr( KD81Data[SamplePositionIdx-1].SOKUTEI_NM );
                }
             }
       ...
       ( SamplePositionIdx >= iFirstPosition && SamplePositionIdx <= iLastPosition ) &&
         DataNoIdx > 2   ){
          ...
          pEdt3[SamplePositionIdx-1]->Text = FormatFloat("#0.0",
                                      KD81Data[SamplePositionIdx-1].SOKUTEI_M[pos-1] );
          ...
          for(int i=0; i<3; i++ ){
             ...
             if( i== 0 )
                 pEdt3_S[i]->Text = FormatFloat("#0.000",
                                      KD81Data[SamplePositionIdx-1].SOKUTEI_S[pos-1] );
             if( i== 1)
                 pEdt3_S[i]->Text = FormatFloat("#0.000",
                                      KD81Data[SamplePositionIdx-1].SOKUTEI_H[pos-1] );
             if( i== 2 )
                 pEdt3_S[i]->Text = FormatFloat("#0.0",
                                      KD81Data[SamplePositionIdx-1].SOKUTEI_K[pos-1] );
          }
          // 更新したデータをチェックする
           CheckInKikaku(0);
   ...
   //  TNumberEdit *Edt = (TNumberEdit *)Sender;
     ...
     //画面に表示しているﾃﾞｰﾀ値を更新される場合
      ...
      CurDataChg = 1;
        ...
        // 計測したデータCount
        if ( KD81Data[SamplePositionIdx-1].SOKUTEI_NM_CUR < DataNoIdx ){
             KD81Data[SamplePositionIdx-1].SOKUTEI_NM_CUR = DataNoIdx ;
        }
   } */
   ...
   // 計測値のチェック
    ...
    //   CurDataChg = 0;
      ...
      if( DataNoIdx == 1 ){
          ...
          a =KD81Data[SamplePositionIdx-1].SOKUTEI_S[0] ;
          if(  fabs(pEdt1_S[0]->Value - a ) > 0.000001){
            ...
            // 計測したデータCount
            if ( KD81Data[SamplePositionIdx-1].SOKUTEI_NS_CUR < DataNoIdx ){
               KD81Data[SamplePositionIdx-1].SOKUTEI_NS_CUR = DataNoIdx ;
            }


            //測定値１
             KD81Data[SamplePositionIdx-1].SOKUTEI_S[0] = pEdt1_S[0]->Value;
             ...
             //2003.02.08 E.Takase
             if ( KD81Data[SamplePositionIdx-1].KEIJYO_KBN == 5 ) { //コンミ
                pEdt1[SamplePositionIdx-1]->Text = FormatFloat("#0.00",KD81Data[SamplePositionIdx-1].SOKUTEI_M[0] );
             } else {
                pEdt1[SamplePositionIdx-1]->Text = FormatFloat("#0.0",KD81Data[SamplePositionIdx-1].SOKUTEI_M[0] );
             }
          }
     }else if( DataNoIdx == 2 ){

          a =KD81Data[SamplePositionIdx-1].SOKUTEI_S[1] ;
          if(  fabs(pEdt2_S[0]->Value - a ) > 0.000001){
            ...
            //測定値２
            KD81Data[SamplePositionIdx-1].SOKUTEI_S[1] = pEdt2_S[0]->Value;

            //抗折力の計算
            MForceCal(SamplePositionIdx, 2 );
            //2003.02.08 E.Takase
            if ( KD81Data[SamplePositionIdx-1].KEIJYO_KBN == 5 ) { //コンミ
               pEdt2[SamplePositionIdx-1]->Text = FormatFloat("#0.00",KD81Data[SamplePositionIdx-1].SOKUTEI_M[1] );
            } else {
               pEdt2[SamplePositionIdx-1]->Text = FormatFloat("#0.0",KD81Data[SamplePositionIdx-1].SOKUTEI_M[1] );
            }
          }
      }else{
          ...
          a = KD81Data[SamplePositionIdx-1].SOKUTEI_S[n-1] ;
          ...
          if( fabs(pEdt3_S[0]->Value - a ) > 0.000001){
             ...
             // 計測したデータCount
             if ( KD81Data[SamplePositionIdx-1].SOKUTEI_NS_CUR < n )
                  KD81Data[SamplePositionIdx-1].SOKUTEI_NS_CUR = n ;

             KD81Data[SamplePositionIdx-1].SOKUTEI_S[n-1] = pEdt3_S[0]->Value;

             //抗折力の計算
             MForceCal(SamplePositionIdx, n ) ;
             //2003.02.08 E.Takase
             if ( KD81Data[SamplePositionIdx-1].KEIJYO_KBN == 5 ) { //コンミ
                pEdt3[SamplePositionIdx-1]->Text = FormatFloat("#0.00",KD81Data[SamplePositionIdx-1].SOKUTEI_M[n-1] );
             } else {
                pEdt3[SamplePositionIdx-1]->Text = FormatFloat("#0.0",KD81Data[SamplePositionIdx-1].SOKUTEI_M[n-1] );
             }
          }
   ...
   // 計測値のチェック
    ...
    //   CurDataChg = 0;
      ...
      if( DataNoIdx == 1 ){
          ...
          a = KD81Data[SamplePositionIdx-1].SOKUTEI_H[0] ;
          ...
          if(  fabs(pEdt1_S[1]->Value - a ) > 0.000001){
            ...
            // 計測したデータCount
            if ( KD81Data[SamplePositionIdx-1].SOKUTEI_NH_CUR < DataNoIdx ){
                 KD81Data[SamplePositionIdx-1].SOKUTEI_NH_CUR = DataNoIdx ;
            }

            //測定値１
             KD81Data[SamplePositionIdx-1].SOKUTEI_H[0] = pEdt1_S[1]->Value;
           ...
           a = KD81Data[SamplePositionIdx-1].SOKUTEI_H[1] ;
          ...
          if(  fabs(pEdt2_S[1]->Value - a ) > 0.000001){
            ...
            //測定値２
            KD81Data[SamplePositionIdx-1].SOKUTEI_H[1] = pEdt2_S[1]->Value;
          ...
          a = KD81Data[SamplePositionIdx-1].SOKUTEI_H[n-1] ;
          ...
          if( fabs(pEdt3_S[1]->Value - a ) > 0.000001){
             ...
             // 計測したデータCount
             if ( KD81Data[SamplePositionIdx-1].SOKUTEI_NH_CUR < n )
                    KD81Data[SamplePositionIdx-1].SOKUTEI_NH_CUR = n ;

              KD81Data[SamplePositionIdx-1].SOKUTEI_H[n-1] = pEdt3_S[1]->Value;
   ...
   // 計測値のチェック
    ...
    //   CurDataChg = 0;
      ...
      if( DataNoIdx == 1 ){
          ...
          a =  KD81Data[SamplePositionIdx-1].SOKUTEI_K[0] ;
          if(  fabs(pEdt1_S[2]->Value - a ) > 0.000001){
            ...
            // 計測したデータCount
            if ( KD81Data[SamplePositionIdx-1].SOKUTEI_NK_CUR < DataNoIdx ){
                 KD81Data[SamplePositionIdx-1].SOKUTEI_NK_CUR = DataNoIdx ;
            }

            //測定値１
            KD81Data[SamplePositionIdx-1].SOKUTEI_K[0] = pEdt1_S[2]->Value;
          ...
          a =  KD81Data[SamplePositionIdx-1].SOKUTEI_K[1] ;
          if(  fabs(pEdt2_S[2]->Value - a ) > 0.000001){
            ...
            //測定値２
            KD81Data[SamplePositionIdx-1].SOKUTEI_K[1] = pEdt2_S[2]->Value;
          ...
          int n=pEdt4_S[2]->Value ;
           ...
           a = KD81Data[SamplePositionIdx-1].SOKUTEI_K[n-1] ;
           ...
           if( fabs(pEdt3_S[2]->Value - a ) > 0.000001){
             ...
             // 計測したデータCount
             if ( KD81Data[SamplePositionIdx-1].SOKUTEI_NK_CUR < n )
                    KD81Data[SamplePositionIdx-1].SOKUTEI_NK_CUR = n ;

              KD81Data[SamplePositionIdx-1].SOKUTEI_K[n-1] = pEdt3_S[2]->Value;
...
void __fastcall TForm1::EdtS4_1Change(TObject *Sender)
{
  ...
  if( iKskStart ) {
         ...
         if( n >= 3 && n <= KD81Data[SamplePositionIdx-1].SOKUTEI_NM ){
            ...
            if( KD81Data[SamplePositionIdx-1].KEIJYO_KBN != 4 )
                pEdt3_S[0]->Text = FormatFloat("#0.000",KD81Data[SamplePositionIdx-1].SOKUTEI_S[n-1] );
            else
                pEdt3_S[0]->Text = "0.000";

             pEdt3_S[1]->Text = FormatFloat("#0.000",KD81Data[SamplePositionIdx-1].SOKUTEI_H[n-1] );

             //2003.02.10 E.Takase
             if( KD81Data[SamplePositionIdx-1].KEIJYO_KBN == 5 ){
                pEdt3_S[2]->Text = FormatFloat("#0.00",KD81Data[SamplePositionIdx-1].SOKUTEI_K[n-1] );
             } else {
                pEdt3_S[2]->Text = FormatFloat("#0.0",KD81Data[SamplePositionIdx-1].SOKUTEI_K[n-1] );
             }
...
void __fastcall TForm1::SetMEMO(void)
{
	...
	for( i=0; i<30; i++ )
    {
		...
		if( KD81Data[i].SAMPLE_P == -1 )
		{
			...
			if ( Trim(KD81Data[i].MEMO) != "" ) {
				//pPnlP[i]->Hint = KD81Data[i].MEMO;
				pEdt1[i]->Hint = KD81Data[i].MEMO;
				pEdt2[i]->Hint = KD81Data[i].MEMO;
				pEdt3[i]->Hint = KD81Data[i].MEMO;
				pEdt4[i]->Hint = KD81Data[i].MEMO;
				pScrl[i]->Hint = KD81Data[i].MEMO;
				pPnlN[i]->Hint = KD81Data[i].MEMO;
