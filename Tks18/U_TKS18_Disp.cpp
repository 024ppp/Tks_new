...
*  file   name   : U_TKS18_DISP.cpp                                    *
*  create        : 1999.11                                             *
*  update        :                                                     *
*  contents      : 抗折力測定GUI処理用関数                             *
     ...
     if(pEdt2_S[0]->Focused() ||
        ...
        if( KD81Data[SamplePositionIdx-1].SOKUTEI_NM > 2 )
        {
            ...
            while( 1 ){
                ...
                if( KD81Data[SamplePositionIdx-1+irec].SOKUTE_KBN  == 1){
                    iFlg = 1;
                    break ;
                 }
             }
     ...
     if(pEdt3_S[0]->Focused() ||
        ...
        // ﾃﾞｰﾀセーブフラグを設定
         iSaveFlag = 1;
          ...
          if( n >= KD81Data[SamplePositionIdx-1].SOKUTEI_NM  ){
            ...
            //データの更新
             pEdt3[SamplePositionIdx-1]->Text = KD81Data[SamplePositionIdx-1].SOKUTEI_M[n] ;
             ...
             if( KD81Data[SamplePositionIdx-1].KEIJYO_KBN != 4 )
                pEdt3_S[0]->Text = FormatFloat("#0.000",KD81Data[SamplePositionIdx-1].SOKUTEI_S[n] );
             else
                pEdt3_S[0]->Text = "0.000";

             pEdt3_S[1]->Text = FormatFloat("#0.000",KD81Data[SamplePositionIdx-1].SOKUTEI_H[n] );

             if( KD81Data[SamplePositionIdx-1].KEIJYO_KBN == 5 ){
                pEdt3_S[2]->Text = FormatFloat("#0.00",KD81Data[SamplePositionIdx-1].SOKUTEI_K[n] );
             } else {
                pEdt3_S[2]->Text = FormatFloat("#0.0",KD81Data[SamplePositionIdx-1].SOKUTEI_K[n] );
             }
...
int iFlg, n;
   ...
   //  FocusがPUSHﾎﾞﾀﾝにいる場合最後ﾎﾟｼﾞｼｮﾝNOに設定
    if(  Form1->BtnF01->Focused() ||
        ...
        if( KD81Data[iLastPosition-1].SOKUTEI_NM > 2 )
    ...
    // ﾃﾞｰﾀ1にFocusがある
     if(pEdt1_S[0]->Focused() ||
        ...
        if( iFlg == 0 ){
              ...
              // 計測データ数 ＞２の場合
              if( KD81Data[SamplePositionIdx-irec-1].SOKUTEI_NM > 2 ){
         ...
         iSaveFlag = 1;
          ...
          if( n <= 3){
                ...
                ToDrawGraph(SamplePositionIdx, iOption);
                 ...
                 // データの更新
                 pEdt3[SamplePositionIdx-1]->Text = KD81Data[SamplePositionIdx-1].SOKUTEI_M[n-1-1] ;
                 pScrl[SamplePositionIdx-1]->Position = n-1 ;
                 pEdt4[SamplePositionIdx-1]->Text = IntToStr( n-1) ;

                 //測定データ明細の表示
                if( KD81Data[SamplePositionIdx-1].KEIJYO_KBN != 4 )
                    pEdt3_S[0]->Text = FormatFloat("#0.000",KD81Data[SamplePositionIdx-1].SOKUTEI_S[n-1-1] );
                else
                    pEdt3_S[0]->Text = "0.000";

                pEdt3_S[1]->Text = FormatFloat("#0.000",KD81Data[SamplePositionIdx-1].SOKUTEI_H[n-1-1] );

                //2003.02.10 E.Takase
                if( KD81Data[SamplePositionIdx-1].KEIJYO_KBN == 5 ){
                   pEdt3_S[2]->Text = FormatFloat("#0.0",KD81Data[SamplePositionIdx-1].SOKUTEI_K[n-1-1] );
                } else {
                   pEdt3_S[2]->Text = FormatFloat("#0.00",KD81Data[SamplePositionIdx-1].SOKUTEI_K[n-1-1] );
                }
                if( KD81Data[SamplePositionIdx-1].KEIJYO_KBN == 4 ){
                     pEdt4_S[0]->Text = "0" ;
                     pPnl_S[0]->Caption = "0" ;
  ...
  int     errflg=0;
    ...
    if( SamplePositionIdx > 0 && (SamplePositionIdx >= iFirstPosition &&
      ...
      if ( CheckKomoku(SamplePositionIdx, DataNoIdx) ){
       ...
       if(( dBuf > KD81Data[SamplePositionIdx-1].MAGE_MAX  ||
           dBuf < KD81Data[SamplePositionIdx-1].MAGE_MIN ) &&
           DataNoIdx <= KD81Data[SamplePositionIdx-1].SOKUTEI_NM_CUR ){
     ...
     if( KD81Data[SamplePositionIdx-1].SOKUTEI_NM > 2 && DataNoIdx >=3  && errflg != 3){
        int k=0;
        for(int  i=2; i< KD81Data[SamplePositionIdx-1].SOKUTEI_NM; i++ ){

             if( ( KD81Data[SamplePositionIdx-1].SOKUTEI_M[i] > KD81Data[SamplePositionIdx-1].MAGE_MAX ||
                   KD81Data[SamplePositionIdx-1].SOKUTEI_M[i] < KD81Data[SamplePositionIdx-1].MAGE_MIN )  &&
                  KD81Data[SamplePositionIdx-1].SOKUTE_KBN == 1 &&
                  DataNoIdx <= KD81Data[SamplePositionIdx-1].SOKUTEI_NM_CUR  ){
...
int  SaveRegaiDt(void )
{
   ...
   if( KD81Data[SamplePositionIdx-1].SOKUTEI_NM > 2  &&
       KD81Data[SamplePositionIdx-1].SOKUTE_KBN == 1 &&  DataNoIdx >  2 )
   {
      ...
      if( fabs(pEdt3_S[iKomoku-1]->Value - a ) > 0.000001){
          ...
          // 計測したデータCount
          if ( KD81Data[SamplePositionIdx-1].SOKUTEI_NS_CUR < n && iKomoku == 1  )
               KD81Data[SamplePositionIdx-1].SOKUTEI_NS_CUR = n ;
          else if ( KD81Data[SamplePositionIdx-1].SOKUTEI_NH_CUR < n && iKomoku == 2  )
                    KD81Data[SamplePositionIdx-1].SOKUTEI_NH_CUR = n ;
          else if ( KD81Data[SamplePositionIdx-1].SOKUTEI_NK_CUR < n && iKomoku == 3  )
         ...
         //抗折力の計算
         if( MForceCal(SamplePositionIdx, n ) ){
              pEdt3[SamplePositionIdx-1]->Text = FormatFloat("#0.0",KD81Data[SamplePositionIdx-1].SOKUTEI_M[n-1] );
          //  if ( KD81Data[SamplePositionIdx-1].SOKUTEI_NM_CUR < n )
          //       KD81Data[SamplePositionIdx-1].SOKUTEI_NM_CUR = n ;
           }
         }
   }
   return 1;
...
{
     Form1->Pnl5->Caption = FormatFloat("#0.0", KD81Data[SamplePositionIdx-1].MAGE_MIN) ;
     Form1->Pnl6->Caption = FormatFloat("#0.0", KD81Data[SamplePositionIdx-1].MAGE_MAX) ;
     if(KD81Data[SamplePositionIdx-1].MAGE_TAN == 1)
        Form1->Pnl6_1->Caption = "Kgf/c㎡" ;
     else if(KD81Data[SamplePositionIdx-1].MAGE_TAN == 2 )
        Form1->Pnl6_1->Caption = " N/c㎡ " ;
     else if( KD81Data[SamplePositionIdx-1].MAGE_TAN == 4 )
        Form1->Pnl6_1->Caption = " N/m㎡ " ;
     else
        Form1->Pnl6_1->Caption = " MPa  " ;

     Form1->Pnl7->Caption = KD81Data[SamplePositionIdx-1].DTKSCOD ;
     Form1->Pnl8->Caption = KD81Data[SamplePositionIdx-1].DTKSHIN ;
     Form1->Pnl9->Caption = KD81Data[SamplePositionIdx-1].LOTNO ;
     Form1->Pnl10->Caption = KD81Data[SamplePositionIdx-1].HINBAN ;
     Form1->Pnl11->Caption = KD81Data[SamplePositionIdx-1].ZISCOD ;
     Form1->Pnl12->Caption = KD81Data[SamplePositionIdx-1].ZISNAM ;

     Form1->Pnl13->Caption = FormatFloat("#0.0", KD81Data[SamplePositionIdx-1].MAGE_SPAN) ;
...
void SetRed(int iFlg)
{
   ...
   for( i=0; i<30 ; i++ ){
      ...
      if( KD81Data[i].SOKUTE_KBN != 1 ) continue ;

      //最初登録のときに規格チェックしない
      if( iFlg == 1 && IsNullSpace( KD81Data[i].ADDYMD ) ) {
          pEdt1[i]->Font->Color = clBlack;
          pEdt2[i]->Font->Color = clBlack;
          pEdt3[i]->Font->Color = clBlack;
          pEdt4[i]->Font->Color == clBlack;
          continue ;
           ...
           //測定値１ (抵抗）
           if(( KD81Data[i].SOKUTEI_M[0] >  KD81Data[i].MAGE_MAX  ||
                KD81Data[i].SOKUTEI_M[0] <  KD81Data[i].MAGE_MIN ) && ! CheckKomoku(i+1, 1) ){
           ...
           //測定値2 (抵抗）
           if( (KD81Data[i].SOKUTEI_M[1] >  KD81Data[i].MAGE_MAX   ||
                KD81Data[i].SOKUTEI_M[1] <  KD81Data[i].MAGE_MIN)  && ! CheckKomoku(i+1, 2)){
           ...
           if( KD81Data[i].SOKUTEI_NM > 2 ){
               ...
               k = 0;
               for( j=2; j< KD81Data[i].SOKUTEI_NM ; j++){

                     //測定値3～ （抵抗）
                    if( ( KD81Data[i].SOKUTEI_M[j] >  KD81Data[i].MAGE_MAX  ||
                          KD81Data[i].SOKUTEI_M[j] <  KD81Data[i].MAGE_MIN ) &&! CheckKomoku(i+1, j+1)){
                         k = 1 ;
                         break ;
                    }
...
/***********************************************************************
*  function name : int  MForceCal(int iSp,int iDt)                     *
*                  +--------------------------------------+            *
...
*                  +--------------------------------------+            *
*  create        : 1999.11                                             *
*  update        : 2002.11.02 コンミ対応 E.Takase                      *
*  contents      : 抗折力の計算                                        *
*  written by    : 呉 亜娟(NKK unics Corp.)                            *
***********************************************************************/
int  MForceCal(int iSp,int iDt)
{
  int ret;
  ...
  //円柱の場合
  if(  KD81Data[iSp-1].KEIJYO_KBN == 4) {
      if( CheckKomoku( iSp, iDt ) == 0  ){

        // kgf/cm**2 || N/cm**2
        if(KD81Data[iSp-1].MAGE_TAN == 1  || KD81Data[iSp-1].MAGE_TAN == 2 ){

          KD81Data[iSp-1].SOKUTEI_M[iDt-1] =
               8.0* KD81Data[iSp-1].SOKUTEI_K[iDt-1]*KD81Data[iSp-1].SOKUTEI_P[iDt-1]/10 ;
          KD81Data[iSp-1].SOKUTEI_M[iDt-1] = KD81Data[iSp-1].SOKUTEI_M[iDt-1]/
                   (3.1415926535*KD81Data[iSp-1].SOKUTEI_H[iDt-1]*KD81Data[iSp-1].SOKUTEI_H[iDt-1]*KD81Data[iSp-1].SOKUTEI_H[iDt-1]/1000.0);
        // N/mm**2
        }else if( KD81Data[iSp-1].MAGE_TAN == 4 ){

          KD81Data[iSp-1].SOKUTEI_M[iDt-1] =
               8.0* KD81Data[iSp-1].SOKUTEI_K[iDt-1]*KD81Data[iSp-1].SOKUTEI_P[iDt-1];
          KD81Data[iSp-1].SOKUTEI_M[iDt-1] = KD81Data[iSp-1].SOKUTEI_M[iDt-1]/
                   (3.1415926535*KD81Data[iSp-1].SOKUTEI_H[iDt-1]*KD81Data[iSp-1].SOKUTEI_H[iDt-1]*KD81Data[iSp-1].SOKUTEI_H[iDt-1] );
          ...
          KD81Data[iSp-1].SOKUTEI_M[iDt-1] =
               8.0* KD81Data[iSp-1].SOKUTEI_K[iDt-1]*KD81Data[iSp-1].SOKUTEI_P[iDt-1]/10 ;
          KD81Data[iSp-1].SOKUTEI_M[iDt-1] = KD81Data[iSp-1].SOKUTEI_M[iDt-1]/
                   (100.0*3.1415926535*KD81Data[iSp-1].SOKUTEI_H[iDt-1]*KD81Data[iSp-1].SOKUTEI_H[iDt-1]*KD81Data[iSp-1].SOKUTEI_H[iDt-1]/1000.0);
        ...
        ret = 1 ;
          ...
          KD81Data[iSp-1].SOKUTEI_M[iDt-1] = 0.0 ;
          ret = 0;
      }

  //コンミの場合
  }else if(  KD81Data[iSp-1].KEIJYO_KBN == 5) {
      if( CheckKomoku( iSp, iDt ) == 0  ){
          KD81Data[iSp-1].SOKUTEI_M[iDt-1] = KD81Data[iSp-1].SOKUTEI_K[iDt-1];
          ret = 1 ;
      }else{
          KD81Data[iSp-1].SOKUTEI_M[iDt-1] = 0.0 ;
          ret = 0;
  ...
  //ブロックの場合
      ...
      if( CheckKomoku( iSp, iDt ) == 0  ){
        ...
        if(KD81Data[iSp-1].MAGE_TAN == 1  || KD81Data[iSp-1].MAGE_TAN == 2 ){
           ...
           KD81Data[iSp-1].SOKUTEI_M[iDt-1] =
               3.0* KD81Data[iSp-1].SOKUTEI_K[iDt-1]*KD81Data[iSp-1].SOKUTEI_P[iDt-1]*0.1 ;
           KD81Data[iSp-1].SOKUTEI_M[iDt-1] = KD81Data[iSp-1].SOKUTEI_M[iDt-1]/
                   (2.0*KD81Data[iSp-1].SOKUTEI_H[iDt-1]*KD81Data[iSp-1].SOKUTEI_S[iDt-1]*KD81Data[iSp-1].SOKUTEI_S[iDt-1]/1000.0);

        // N/mm**2
        }else if( KD81Data[iSp-1].MAGE_TAN == 4 ){
           KD81Data[iSp-1].SOKUTEI_M[iDt-1] =
               3.0* KD81Data[iSp-1].SOKUTEI_K[iDt-1]*KD81Data[iSp-1].SOKUTEI_P[iDt-1];
           KD81Data[iSp-1].SOKUTEI_M[iDt-1] = KD81Data[iSp-1].SOKUTEI_M[iDt-1]/
                   (2.0*KD81Data[iSp-1].SOKUTEI_H[iDt-1]*KD81Data[iSp-1].SOKUTEI_S[iDt-1]*KD81Data[iSp-1].SOKUTEI_S[iDt-1] );
          ...
          KD81Data[iSp-1].SOKUTEI_M[iDt-1] =
               3.0* KD81Data[iSp-1].SOKUTEI_K[iDt-1]*KD81Data[iSp-1].SOKUTEI_P[iDt-1]*0.1 ;
          KD81Data[iSp-1].SOKUTEI_M[iDt-1] = KD81Data[iSp-1].SOKUTEI_M[iDt-1]/
                   (200.0*KD81Data[iSp-1].SOKUTEI_H[iDt-1]*KD81Data[iSp-1].SOKUTEI_S[iDt-1]*KD81Data[iSp-1].SOKUTEI_S[iDt-1]/1000.0);
        }
  ...
  // 抵抗率計測数
  if( ret){
      if ( KD81Data[iSp-1].SOKUTEI_NM_CUR < iDt )
           KD81Data[iSp-1].SOKUTEI_NM_CUR = iDt ;
  }
...
int CheckKomoku(int iSp, int iDt)
{
  ...
  //円柱
  if( KD81Data[iSp-1].KEIJYO_KBN == 4 ) {
     if( KD81Data[iSp-1].SOKUTEI_NP_CUR == 0  &&
         KD81Data[iSp-1].SOKUTEI_NH_CUR == 0  &&
         KD81Data[iSp-1].SOKUTEI_NK_CUR == 0  ){
         ret = 1 ;
         return ret;
     }

     if(   KD81Data[iSp-1].SOKUTEI_H[iDt-1] <= 0.0000001 ||
           KD81Data[iSp-1].SOKUTEI_K[iDt-1] <= 0.0000001 ||
           KD81Data[iSp-1].SOKUTEI_P[iDt-1] <= 0.0000001    ) {
           ret = 1;
          return ret ;
     }
  //コンミの場合
  } else if ( KD81Data[iSp-1].KEIJYO_KBN == 5 ) {
     if( KD81Data[iSp-1].SOKUTEI_NS_CUR == 0  &&
         KD81Data[iSp-1].SOKUTEI_NK_CUR == 0  ){
         ret = 1 ;
         return ret;
     }

     if(   KD81Data[iSp-1].SOKUTEI_S[iDt-1] <= 0.0000001 ||
           KD81Data[iSp-1].SOKUTEI_K[iDt-1] <= 0.0000001    ) {
           ret = 1;
          return ret ;
     }
  // 長方体の場合
     ...
     if( KD81Data[iSp-1].SOKUTEI_NP_CUR == 0  &&
         KD81Data[iSp-1].SOKUTEI_NS_CUR == 0  &&
         KD81Data[iSp-1].SOKUTEI_NH_CUR == 0  &&
         KD81Data[iSp-1].SOKUTEI_NK_CUR == 0  ){
         ret = 1 ;
         return ret;
     ...
     if(   KD81Data[iSp-1].SOKUTEI_S[iDt-1] <= 0.0000001 ||
           KD81Data[iSp-1].SOKUTEI_H[iDt-1] <= 0.0000001 ||
           KD81Data[iSp-1].SOKUTEI_K[iDt-1] <= 0.0000001 ||
           KD81Data[iSp-1].SOKUTEI_P[iDt-1] <= 0.0000001    ) {
           ret = 1;
          return ret ;
...
int IsKeisokuEnd(void)
{
  ...
  for( i=0; i<30; i++ ){
         ...
         if( (KD81Data[i].SOKUTEI_NM_CUR < KD81Data[i].SOKUTEI_NM ) &&

              KD81Data[i].SOKUTE_KBN ){
