   ...
   int DataNoIdx_Old;
     ...
     for( int i=0; i< KD81Data[SamplePositionIdx-1].SOKUTEI_NM; i++ ){
          ...
          if( MForceCal(SamplePositionIdx, i+1 ) ){
              ...
              //2003.02.08 E.Takase
              if ( KD81Data[SamplePositionIdx-1].KEIJYO_KBN == 5 ) { //ƒRƒ“ƒ~
                 if( i == 0)
                    pEdt1[SamplePositionIdx-1]->Text = FormatFloat("#0.00",KD81Data[SamplePositionIdx-1].SOKUTEI_M[i] );
                 else if( i == 1)
                    pEdt2[SamplePositionIdx-1]->Text = FormatFloat("#0.00",KD81Data[SamplePositionIdx-1].SOKUTEI_M[i] );
                 else
                    pEdt3[SamplePositionIdx-1]->Text = FormatFloat("#0.00",KD81Data[SamplePositionIdx-1].SOKUTEI_M[i] );
               } else {
                 if( i == 0)
...
void __fastcall TForm4::FormActivate(TObject *Sender)
{
     ...
     EdtS->Text    = FormatFloat( "#0.0", KD81Data[SamplePositionIdx-1].SOKUTEI_P[0] );
     EdtS->SetFocus();
     EdtS->SelectAll();
}
//---------------------------------------------------------------------------
