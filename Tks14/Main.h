...
extern struct structKD41{
      ...
      double LD_HP_MIN ;       //引張強度 規格下
      int    LD_HP_SPD;        //引張強度 測定速度
      int    LD_HP_TAN;        //単位記号
      int SOKUTEI_NM ;         //引張強度測定個数
      int SOKUTEI_SU ;         //引張強度測定個数
...
//リード線引張強度履歴作画用テーブル変数 KD43W
extern struct structKD43W{
      AnsiString DTKSHIN;          //得意先品番
      AnsiString LOTNO[50];        //ロットNO
      double LD_HP_MIN[50] ;       //リード線引張強度 規格下
