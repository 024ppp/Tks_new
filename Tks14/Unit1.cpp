...
//---------------------------------------------------------------------------
// KD41構造体変数
structKD41 KD41Data[30];      // ﾘｰﾄﾞ線引張ﾃﾞｰﾀﾌｧｲﾙ  KD41

//KD43W構造体変数
structKD43W KD43WData[30];    // ﾘｰﾄﾞ線引張履歴ﾃﾞｰﾀﾜｰｸﾌｧｲﾙ  KD43W


//---------------------------------------------------------------------------
//画面操作用変数
int iRec_Kd41;              // 一覧検索結果件数
...
long  Kd43W_Create01(AnsiString, int, int) ;
int  Kd43W_Serch01();
...
void ExecF1(void)
{
    if( Form1->edtymd->Focused() || Form1->edttlno->Focused() ){
     ...
     //リード線引張強度ﾃﾞｰﾀﾌｧｲﾙからデータを抽出
     iRec_Kd41 = Kd41_Serch01() ;
...
void __fastcall TForm1::BtnF07Click(TObject *Sender)
{
   ...
   iKskStart = 0 ;
    ...
    // リード線引張ﾃﾞｰﾀﾌｧｲﾙからﾃﾞｰﾀを抽出  KD41
    int iRtn = Kd41_Serch02();
    if( iRtn <= 0 ) {
	...
	if( CheckBoxRireki->Checked ){
	     ...
	     //Kd43W履歴ﾃﾞｰﾀテーブルの作成
	     //int iRec = Kd43W_Create01(KD41Data[0].KENSA_YMD, KD41Data[0].TOLEY_NO, LOTMAX);
	     int iRec = Kd43W_Create01(strVal01, strVal02.ToIntDef(0), LOTMAX);
	     
		if( iRec >0 ){

	        //Kd43W履歴ﾃﾞｰﾀﾌｧｲﾙを読み込む
	        i  =  Kd43W_Serch01();
          ...
          if( KD41Data[i].SAMPLE_P == -1 )
          {
             ...
             /*  IsEnabledEdt( pEdt1[i], 0 );
               ...
               */
                ...
                //ﾃﾞｰﾀ１、ﾃﾞｰﾀ２の設定
               pEdt1[i]->Text =  FormatFloat("#0.0",KD41Data[i].SOKUTEICHI[0] );
               pEdt2[i]->Text =  FormatFloat("#0.0",KD41Data[i].SOKUTEICHI[1] );
               pPnlN[i]->Caption = IntToStr( KD41Data[i].SOKUTEI_NM) ;

               // 例外ﾃﾞｰﾀ の設定
               if( KD41Data[i].SOKUTEI_NM > 2 )
               {
                 pEdt3[i]->Text =  FormatFloat("#0.0",KD41Data[i].SOKUTEICHI[2] );
                ...
                if(  KD41Data[i].SOKUTE_KBN == 1 ){
                  ...
                  if( KD41Data[i].SOKUTEI_NM > 2 )
                  {
                    ...
                    // ScrollBarの初期値設定
                    pScrl[i]->Min = 3;
                    pScrl[i]->Max = KD41Data[i].SOKUTEI_NM;
                    pScrl[i]->Position = 3 ;
                  ...
                  iLastPosition = KD41Data[i].SAMPLE_P ;
             }else{
                   // ﾃﾞｰﾀ１、ﾃﾞｰﾀ２ｺﾝﾄﾛｰﾙ
                /*   IsEnabledEdt( pEdt1[i], 0 );
                   IsEnabledEdt( pEdt2[i], 0 );
      ...
      TShiftState Shift)
{
     ...
     switch(Key)
     {
         ...
         case VK_F3:  ToNextData();           break;
         case VK_F5:  BtnF05Click(Sender);    break;
         case VK_F6:  BtnF06Click(Sender);    break;       //2002.09.17
         case VK_F7:
       ...
       ( SamplePositionIdx >= iFirstPosition && SamplePositionIdx <= iLastPosition ) &&
         DataNoIdx > 2   ){
          ...
          pEdt3[SamplePositionIdx-1]->Text = FormatFloat("#0.0",
                                      KD41Data[SamplePositionIdx-1].SOKUTEICHI[pos-1] );
...
void __fastcall TForm1::Edt01_1Change(TObject *Sender)
{
    ...
    // TNumberEdit *Edt = (TNumberEdit *)Sender;
     ...
     //画面に表示しているﾃﾞｰﾀ値を更新される場合
        ...
        // 計測したデータCount
        if ( KD41Data[SamplePositionIdx-1].SOKUTEI_NM_CUR < DataNoIdx ){
             KD41Data[SamplePositionIdx-1].SOKUTEI_NM_CUR = DataNoIdx ;
       }
   }
}
...
void __fastcall TForm1::Edt01_4Change(TObject *Sender)
{
  ...
  if( iKskStart ) {
     ...
     if( n >= 3 && n <= KD41Data[SamplePositionIdx-1].SOKUTEI_NM ){
         ...
         pEdt3[SamplePositionIdx-1]->Text = FormatFloat("#0.0",KD41Data[SamplePositionIdx-1].SOKUTEICHI[n-1] );
...
void __fastcall TForm1::SetMEMO(void)
{
	...
	for( i=0; i<30; i++ )
    {
		...
		if( KD41Data[i].SAMPLE_P == -1 )
		{
			...
			if ( Trim(KD41Data[i].MEMO) != "" ) {
				//pPnlP[i]->Hint = KD41Data[i].MEMO;
				pEdt1[i]->Hint = KD41Data[i].MEMO;
				pEdt2[i]->Hint = KD41Data[i].MEMO;
				pEdt3[i]->Hint = KD41Data[i].MEMO;
				pEdt4[i]->Hint = KD41Data[i].MEMO;
				pScrl[i]->Hint = KD41Data[i].MEMO;
				pPnlN[i]->Hint = KD41Data[i].MEMO;
