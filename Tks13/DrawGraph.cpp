...
*  create        : 1999.11                                             *
*  update        :                                                     *
*  contents      : ﾘｰﾄﾞ線取付抵抗測定履歴、工程能力作画関数            *
...
void DispKoutei( int iSp )
{
   ...
   //表示データを求める
   int iLinkSp;
   if(  KD31Data[iSp-1].LINK == 0 ){
       iLinkSp = iSp ;
   }else {
       iLinkSp = KD31Data[iSp-1].LINK;
   }
   ...
   for( i=iLotStart; i < iRtn ; i++ ) {
          ...
          if( KD33WData[iLinkSp-1].SOKUTEI_NM[i] == 0 ) continue ;

           //履歴データ個数
           n += KD33WData[iLinkSp-1].SOKUTEI_NM[i] ;

           // 平均値の計算
           dAve += KD33WData[iLinkSp-1].SOKUTE_AVE[i]*KD33WData[iLinkSp-1].SOKUTEI_NM[i];

           for( j=0; j< KD33WData[iLinkSp-1].SOKUTEI_NM[i]; j++ ) {

                if( KD33WData[iLinkSp-1].SOKUTEI[i][j] > dMax ) dMax = KD33WData[iLinkSp-1].SOKUTEI[i][j];
                if( KD33WData[iLinkSp-1].SOKUTEI[i][j] < dMin ) dMin = KD33WData[iLinkSp-1].SOKUTEI[i][j];

                //不合格ﾃﾞｰﾀのﾁｪｯｸ
                if(  KD33WData[iLinkSp-1].SOKUTEI[i][j] >  KD33WData[iLinkSp-1].LD_TR_MAX[i]  )
                 FuryoCnt +=1 ;
   ...
   //ｶﾚﾝﾄｶｰｿﾙがある計測ﾃﾞｰﾀ
   if( KD31Data[iSp-1].SOKUTEI_NM_CUR != 0 )
   {
            //データ個数
            n += KD31Data[iSp-1].SOKUTEI_NM_CUR ;

            // 平均値の計算
            for( j=0; j< KD31Data[iSp-1].SOKUTEI_NM_CUR; j++ ) {
                dAve += KD31Data[iSp-1].SOKUTEICHI[j];

               if( KD31Data[iSp-1].SOKUTEICHI[j] > dMax ) dMax = KD31Data[iSp-1].SOKUTEICHI[j];
               if( KD31Data[iSp-1].SOKUTEICHI[j] < dMin ) dMin = KD31Data[iSp-1].SOKUTEICHI[j];

                //不合格ﾃﾞｰﾀのﾁｪｯｸ
                if(  KD31Data[iSp-1].SOKUTEICHI[j] >  KD31Data[iSp-1].LD_TR_MAX )
                 FuryoCnt +=1 ;
           }
   ...
   if( n!= 1 ){
       ...
       for( i=iLotStart; i<iRtn ; i++ )
       {
           for( int j=0; j< KD33WData[iLinkSp-1].SOKUTEI_NM[i]; j++ )
                dSigma += (KD33WData[iLinkSp-1].SOKUTEI[i][j] - dAve)*(KD33WData[iLinkSp-1].SOKUTEI[i][j] - dAve) ;
       }

       if( KD31Data[iSp-1].SOKUTEI_NM_CUR != 0 ) {
           for( i=0; i<KD31Data[iSp-1].SOKUTEI_NM_CUR ; i++ )
                dSigma += (KD31Data[iSp-1].SOKUTEICHI[i] - dAve)*(KD31Data[iSp-1].SOKUTEICHI[i] - dAve) ;
       }
       ...
       // Cp
       if( dSigma > 0.0 )
           dCp = fabs(KD31Data[iSp-1].LD_TR_MAX - dAve)/(3*dSigma) ;
       else
           dCp = 0.0 ;
...
void SetRirekiData(int iSp)
{
  ...
  // 作画用変数の初期化
   ...
   for( i=iLotStart; i < iRtn ; i++ ) {
          ...
          if( KD33WData[iLinkSp-1].SOKUTEI_NM[i] != 0 ) {

              // 最大値、最小値、平均値の計算
              Max[i-iLotStart] = maxa;
              Min[i-iLotStart] = mina ;
              for(  j=0; j<KD33WData[iLinkSp-1].SOKUTEI_NM[i] ; j++ ){
                    if( KD33WData[iLinkSp-1].SOKUTEI[i][j] > Max[i-iLotStart] ) Max[i-iLotStart] = KD33WData[iLinkSp-1].SOKUTEI[i][j];
                    if( KD33WData[iLinkSp-1].SOKUTEI[i][j] < Min[i-iLotStart] ) Min[i-iLotStart] = KD33WData[iLinkSp-1].SOKUTEI[i][j];

                    Val[i-iLotStart] += KD33WData[iLinkSp-1].SOKUTEI[i][j] ;
              }

              Val[i-iLotStart] = Val[i-iLotStart]/KD33WData[iLinkSp-1].SOKUTEI_NM[i] ;
          }

          str = KD33WData[iLinkSp-1].LOTNO[i] ;
   ...
   if( KD31Data[iSp-1].SOKUTEI_NM_CUR != 0 )
   {
         ...
         Min[iRRR] = mina ;
         for( i=0; i< KD31Data[iSp-1].SOKUTEI_NM_CUR ; i++ ){
              if( KD31Data[iSp-1].SOKUTEICHI[i] > Max[iRRR] ) Max[iRRR] = KD31Data[iSp-1].SOKUTEICHI[i];
              if( KD31Data[iSp-1].SOKUTEICHI[i] < Min[iRRR] ) Min[iRRR] = KD31Data[iSp-1].SOKUTEICHI[i];
         }

         // 平均値の計算
         for( i=0; i< KD31Data[iSp-1].SOKUTEI_NM_CUR; i++ )
              Val[iRRR] += KD31Data[iSp-1].SOKUTEICHI[i] ;
         Val[iRRR] = Val[iRRR]/KD31Data[iSp-1].SOKUTEI_NM_CUR ;
   }

   str = KD31Data[iSp-1].LOTNO ;
   if( str.Length() > 5 ) str = str.Delete(1,1);
...
*  create        : 1999.11                                             *
*  update        :                                                     *
*  contents      : KD33Wから読み込んだﾃﾞｰﾀ中にに指定したロットNo       *
*                :  有るかどうか のチェック                            *
*  written by    : 呉 亜娟(NKK unics Corp.)                            *
***********************************************************************/
int  CheckLotNo(int iSp)
{
   ...
   for( int i=0; i<  KD33WData[iLinkSp-1].LOTCNT; i++ ){
      if(  KD31Data[iSp-1].LOTNO ==  KD33WData[iLinkSp-1].LOTNO[i] )
      {
        iFlag = i ;
        break ;
       }
