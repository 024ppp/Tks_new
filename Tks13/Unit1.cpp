...
//---------------------------------------------------------------------------
// KD31構造体変数
structKD31 KD31Data[30];      // ﾘｰﾄﾞ線取付抵抗ﾃﾞｰﾀﾌｧｲﾙ  KD31

//KD33W構造体変数
structKD33W KD33WData[30];    // ﾘｰﾄﾞ線取付抵抗履歴ﾃﾞｰﾀﾜｰｸﾌｧｲﾙ  KD33W


//---------------------------------------------------------------------------
//画面操作用変数
int iRec_Kd31;              // 一覧検索結果件数
...
int AllDataChg;            // 画面に表示しているﾃﾞｰﾀ更新されるかどうか
int CurSampChg;
int CurDataChg;
...
AnsiString MSG_13 = "計測機器の応答がありません。　検索及びキーボードからの入力は使用可能です。" ;
AnsiString MSG_14 = "計測機器は他のパソコンで使用しています。　検索及びキーボードからの入力は使用可能です。" ;
...
long  Kd33W_Create01(AnsiString, int, int) ;
int  Kd33W_Serch01();
int IsNullSpace(AnsiString);
int GetItem(char **buff, char *item) ;
     ...
     if (iShareFlag == 2) {
        ...
        if( nNumber > 0 &&  strncmp( SharedBuff, "OK", 2) != 0 &&
           ...
           if( DataNoIdx == 1 ) {
               ...
               //測定値１のセット
               EdtV1->Text = FormatFloat("#0.0", dV);
               EdtA1->Text = FormatFloat("#0.0", dA);

           }else if (DataNoIdx == 2 ){
               //測定値２のセット
               EdtV2->Text = FormatFloat("#0.0", dV);
               EdtA2->Text = FormatFloat("#0.0", dA);

           }else{
               // 測定値NO＞２の場合ﾃﾞｰﾀのセット
               EdtV3->Text = FormatFloat("#0.0", dV);
               EdtA3->Text = FormatFloat("#0.0", dA);
           }
...
void ExecF1(void)
{
   if( Form1->edtymd->Focused() || Form1->edttlno->Focused() ){
     ...
     //リード線取付抵抗ﾃﾞｰﾀﾌｧｲﾙからデータを抽出
     iRec_Kd31 = Kd31_Serch01() ;
...
void __fastcall TForm1::BtnF07Click(TObject *Sender)
{
   ...
   iKskStart = 0 ;
    ...
    // リード線取付抵抗ﾃﾞｰﾀﾌｧｲﾙからﾃﾞｰﾀを抽出  KD31
    int iRtn = Kd31_Serch02();
    if( iRtn <= 0 ) {
    ...
    // 計測値の変更があったかどうかの指示フラグ
     ...
     ////KD33W履歴ﾃﾞｰﾀテーブルの作成
     //int iRec = Kd33W_Create01(KD31Data[0].KENSA_YMD, KD31Data[0].TOLEY_NO, LOTMAX);
     ////if( iRec >0 ){
     //
     //   //KD33W履歴ﾃﾞｰﾀﾌｧｲﾙを読み込む
     //   i  =  Kd33W_Serch01();
     //
     //// }else{
     //  if( iRec <= 0) StatBarClear( MSG_12) ;
     //// }
	 ...
	 if( CheckBoxRireki->Checked ){
	     ...
	     //KD33W履歴ﾃﾞｰﾀテーブルの作成
	     //int iRec = Kd33W_Create01(KD31Data[0].KENSA_YMD, KD31Data[0].TOLEY_NO, LOTMAX);
	     int iRec = Kd33W_Create01(strVal01, strVal02.ToIntDef(0), LOTMAX);
...
//        ToDrawGraph(1, iOption);
          ...
          if( KD31Data[i].SAMPLE_P == -1 )
          {
           ...
           /*    IsEnabledEdt( pEdt1[i], 0 );
             ...
             */
                ...
                //ﾃﾞｰﾀ１、ﾃﾞｰﾀ２の設定
               pEdt1[i]->Text =  FormatFloat("#0.0",KD31Data[i].SOKUTEICHI[0] );
               pEdt2[i]->Text =  FormatFloat("#0.0",KD31Data[i].SOKUTEICHI[1] );
               pPnlN[i]->Caption = IntToStr( KD31Data[i].SOKUTEI_NM) ;

               // 例外ﾃﾞｰﾀ の設定
               if( KD31Data[i].SOKUTEI_NM > 2 )
               {
                 pEdt3[i]->Text =  FormatFloat("#0.0",KD31Data[i].SOKUTEICHI[2] );
               ...
               if(  KD31Data[i].SOKUTE_KBN == 1 ){
                  ...
                  if( KD31Data[i].SOKUTEI_NM > 2 )
                  {
                    ...
                    pScrl[i]->Min = 3;
                    pScrl[i]->Position = 3 ;
                    pScrl[i]->Max = KD31Data[i].SOKUTEI_NM;
                  ...
                  if( flag == 0 && HaveSokutei){
                   ...
                   iLastPosition = KD31Data[i].SAMPLE_P ;
    ...
    if( HaveSokutei ){
         ...
         // 測定データ明細のﾎﾞﾀﾝを有効にする
         IsEnabledEdt( EdtV1, 1 );
         IsEnabledEdt( EdtV2, 1 );
         IsEnabledEdt( EdtV3, 1 );
         IsEnabledEdt( EdtV4, 1 );
         IsEnabledPnl( PnlV, 1 );
         IsEnabledScrl( ScrollBarV,1 );

         IsEnabledEdt( EdtA1, 1 );
         IsEnabledEdt( EdtA2, 1 );
         IsEnabledEdt( EdtA3, 1 );
         IsEnabledEdt( EdtA4, 1 );
         IsEnabledPnl( PnlA, 1 );
         IsEnabledScrl( ScrollBarA,1 );


         // 測定条件指示のﾎﾞﾀﾝを有効にする
         IsEnabledPnl( PnlD, 1 );
         IsEnabledPnl( PnlT, 1 );
         IsEnabledPnl( PnlTYP, 1 );
...
void __fastcall TForm1::SetEnabel(int flag )
{
  if( flag == 0 ){
      ...
      for( int i=0; i<30; i++ )
      {
       ...
       //計測明細ｺﾝﾄﾛｰﾙ
       IsEnabledEdt( EdtV1, 0 );
       IsEnabledEdt( EdtV2, 0 );
       IsEnabledEdt( EdtV3, 0 );
       IsEnabledEdt( EdtV4, 0 );
       IsEnabledPnl( PnlV, 0 );
       IsEnabledScrl( ScrollBarV,0 );

       IsEnabledEdt( EdtA1, 0 );
       IsEnabledEdt( EdtA2, 0 );
       IsEnabledEdt( EdtA3, 0 );
       IsEnabledEdt( EdtA4, 0 );
       IsEnabledPnl( PnlA, 0 );
       IsEnabledScrl( ScrollBarA,0 );

       //計測条件
       IsEnabledPnl( PnlD, 0 );
       IsEnabledPnl( PnlT, 0 );
       IsEnabledPnl( PnlTYP, 0 );
...
void __fastcall TForm1:: OnEditEnter( int spflag, int dtflag, TNumberEdit *pEdit )
{
    ...
    //測定データ明細の表示
    EdtV1->Text =  FormatFloat("#0.0",KD31Data[SamplePositionIdx-1].SOKUTEI_V[0] );
    EdtV2->Text =  FormatFloat("#0.0",KD31Data[SamplePositionIdx-1].SOKUTEI_V[1] );
    EdtA1->Text =  FormatFloat("#0.0",KD31Data[SamplePositionIdx-1].SOKUTEI_A[0] );
    EdtA2->Text =  FormatFloat("#0.0",KD31Data[SamplePositionIdx-1].SOKUTEI_A[1] );

    //電流、電圧測定個数の設定
    PnlV->Caption = IntToStr( KD31Data[SamplePositionIdx-1].SOKUTEI_NM );
    PnlA->Caption = IntToStr( KD31Data[SamplePositionIdx-1].SOKUTEI_NM );

    //測定個数＞２の場合
    if( KD31Data[SamplePositionIdx-1].SOKUTE_KBN == 1 &&
        KD31Data[SamplePositionIdx-1].SOKUTEI_NM  > 2 ){
             ...
             //抵抗に合わせて電流、電圧のScrollBarを設定する
             ScrollBarV->Min = 3;
             ScrollBarV->Max = KD31Data[SamplePositionIdx-1].SOKUTEI_NM;
             ScrollBarV->Position = pos ;
             EdtV3->Text =  FormatFloat("#0.0",KD31Data[SamplePositionIdx-1].SOKUTEI_V[pos-1] );
             EdtV4->Text = IntToStr(pos) ;

             ScrollBarA->Min = 3;
             ScrollBarA->Position = pos ;
             ScrollBarA->Max = KD31Data[SamplePositionIdx-1].SOKUTEI_NM;
             EdtA3->Text =  FormatFloat("#0.0",KD31Data[SamplePositionIdx-1].SOKUTEI_A[pos-1] );
             EdtA4->Text = IntToStr(pos) ;
...
void __fastcall TForm1::EdtV1Enter(TObject *Sender)
{
    ...
    if( DataNoIdx == 1 ){
     ...
     // ﾃﾞｰﾀNOの設定
    if( EdtV1->Focused() || EdtA1->Focused()  ) DataNoIdx = 1;
    if( EdtV2->Focused() || EdtA2->Focused()  ) DataNoIdx = 2;
    if( EdtV3->Focused() || EdtA3->Focused() ||
        EdtV4->Focused() || EdtA4->Focused() ||
        ScrollBarV->Focused() ||
        ScrollBarA->Focused()                  ) DataNoIdx= EdtV4->Value ;

    if( DataNoIdx == 1 ){
       if( EdtV1->Focused() ) VFlg = 1;
       if( EdtA1->Focused() ) VFlg = 2;
    }else if( DataNoIdx == 2 ){
       if( EdtV2->Focused() ) VFlg = 1;
       if( EdtA2->Focused() ) VFlg = 2;
    }else {
       if( EdtV3->Focused() || EdtV4->Focused() || ScrollBarV->Focused() ) VFlg = 1;
       if( EdtA3->Focused() || EdtA4->Focused() || ScrollBarA->Focused() ) VFlg = 2;
    }
       ...
       ( SamplePositionIdx >= iFirstPosition && SamplePositionIdx <= iLastPosition ) &&
         DataNoIdx > 2   ){
          ...
          pEdt3[SamplePositionIdx-1]->Text = FormatFloat("#0.0",
                                      KD31Data[SamplePositionIdx-1].SOKUTEICHI[pos-1] );
         ...
         // pEdt3[SamplePositionIdx-1]->SetFocus();
          ...
          EdtV3->Text = FormatFloat("#0.0",
                                      KD31Data[SamplePositionIdx-1].SOKUTEI_V[pos-1] );
          EdtA3->Text = FormatFloat("#0.0",
                                      KD31Data[SamplePositionIdx-1].SOKUTEI_A[pos-1] );
   ...
   if( SamplePositionIdx >0 &&
       ( SamplePositionIdx >= iFirstPosition && SamplePositionIdx <= iLastPosition ) &&
         DataNoIdx > 2   ){
          ...
          //電流を同時に更新
          ScrollBarA->Position = pos;
          pScrl[SamplePositionIdx-1]-> Position = pos ;
...
void __fastcall TForm1::ScrollBarAChange(TObject *Sender)
{
   //計測作業始まる前 、つまり「検索」ﾎﾞﾀﾝを押す前
   if( ! iKskStart )  return ;
...
//---------------------------------------------------------------------------
 void __fastcall TForm1::EdtA1Change(TObject *Sender)
{
/*    if( iKskStart ) {
     ...
     // 計測したデータCount
     if ( KD31Data[SamplePositionIdx-1].SOKUTEI_NA_CUR < DataNoIdx ){
             KD31Data[SamplePositionIdx-1].SOKUTEI_NA_CUR = DataNoIdx ;
     }
   } */
}
//---------------------------------------------------------------------------
...
/*    if( iKskStart ) {
    ...
    // TNumberEdit *Edt = (TNumberEdit *)Sender;
     ...
     // 計測したデータCount
     if ( KD31Data[SamplePositionIdx-1].SOKUTEI_NM_CUR < DataNoIdx ){
             KD31Data[SamplePositionIdx-1].SOKUTEI_NM_CUR = DataNoIdx ;
     }
   } */
...
void __fastcall TForm1::EdtA1Exit(TObject *Sender)
{
    if( CurBackFlg ) return ;
   ...
   // 計測値のチェック
     if( iKskStart && ! CurBackFlg && SamplePositionIdx >= 1 ) {
      ...
      if( DataNoIdx == 1 ){
         ...
         if( fabs(KD31Data[SamplePositionIdx-1].SOKUTEI_V[0] - EdtV1->Value)>0.000001 ||
             fabs(KD31Data[SamplePositionIdx-1].SOKUTEI_A[0] - EdtA1->Value)>0.000001 ){
             ...
             if( fabs(KD31Data[SamplePositionIdx-1].SOKUTEI_V[0] - EdtV1->Value)>0.000001 ) {
                if ( KD31Data[SamplePositionIdx-1].SOKUTEI_NM_CUR < DataNoIdx )
                    KD31Data[SamplePositionIdx-1].SOKUTEI_NM_CUR = DataNoIdx ;
                //電圧
                KD31Data[SamplePositionIdx-1].SOKUTEI_V[0] = EdtV1->Value;
             }

             if( fabs(KD31Data[SamplePositionIdx-1].SOKUTEI_A[0] - EdtA1->Value)>0.000001 ){
               if ( KD31Data[SamplePositionIdx-1].SOKUTEI_NA_CUR < DataNoIdx )
                    KD31Data[SamplePositionIdx-1].SOKUTEI_NA_CUR = DataNoIdx ;
                //電流
                KD31Data[SamplePositionIdx-1].SOKUTEI_A[0] = EdtA1->Value;
             }

             KD31Data[SamplePositionIdx-1].SOKUTEICHI[0]= EdtV1->Value;

             //計算した抵抗値を画面へセット
             pEdt1[SamplePositionIdx-1]->Text = FormatFloat("#0.0",KD31Data[SamplePositionIdx-1].SOKUTEICHI[0] );
        }
      }else if( DataNoIdx == 2 ){

         if( fabs(KD31Data[SamplePositionIdx-1].SOKUTEI_V[1] - EdtV2->Value)>0.000001 ||
             fabs(KD31Data[SamplePositionIdx-1].SOKUTEI_A[1] - EdtA2->Value)>0.000001 ){
             ...
             if( fabs(KD31Data[SamplePositionIdx-1].SOKUTEI_V[1] - EdtV2->Value)>0.000001 ) {
                if ( KD31Data[SamplePositionIdx-1].SOKUTEI_NM_CUR < DataNoIdx )
                    KD31Data[SamplePositionIdx-1].SOKUTEI_NM_CUR = DataNoIdx ;
                //電圧
                KD31Data[SamplePositionIdx-1].SOKUTEI_V[1] = EdtV2->Value;
             }

             if( fabs(KD31Data[SamplePositionIdx-1].SOKUTEI_A[1] - EdtA2->Value)>0.000001 ){
               if ( KD31Data[SamplePositionIdx-1].SOKUTEI_NA_CUR < DataNoIdx )
                    KD31Data[SamplePositionIdx-1].SOKUTEI_NA_CUR = DataNoIdx ;
                //電流
                KD31Data[SamplePositionIdx-1].SOKUTEI_A[1] = EdtA2->Value;
             }

             KD31Data[SamplePositionIdx-1].SOKUTEICHI[1] = EdtV2->Value;
             //計算した抵抗値を画面へセット
             pEdt2[SamplePositionIdx-1]->Text = FormatFloat("#0.0",KD31Data[SamplePositionIdx-1].SOKUTEICHI[1] );
        }
       ...
       /*
          ...
          if( iSaveFlag ) return ;
       */
         if(  fabs(EdtV3->Value - KD31Data[SamplePositionIdx-1].SOKUTEI_V[n-1]) > 0.0000001 ||
              fabs(EdtA3->Value - KD31Data[SamplePositionIdx-1].SOKUTEI_A[n-1] ) > 0.0000001){
              ...
              if( fabs(EdtA3->Value - KD31Data[SamplePositionIdx-1].SOKUTEI_A[n-1]) > 0.0000001 ){
...
void __fastcall TForm1::EdtV4Change(TObject *Sender)
{
  int n;
  ...
  if( iKskStart ) {
     ...
     if( VFlg == 1 ){
         ...
         if( n >= 3 && n <= KD31Data[SamplePositionIdx-1].SOKUTEI_NM ){
            ...
            if( n== ScrollBarV->Position || n== 0 ) return ;
            ...
            EdtV3->Text =  FormatFloat("#0.0",KD31Data[SamplePositionIdx-1].SOKUTEI_V[n-1] );
            ScrollBarV->Position = n ;

            EdtA3->Text =  FormatFloat("#0.0",KD31Data[SamplePositionIdx-1].SOKUTEI_A[n-1] );
            EdtA4->Text = IntToStr(n) ;
            ScrollBarA->Position = n ;
         ...
         //2004.05.13 E.Takase SOKUTEI_NMの値 チェック
         if( n >= 3 && n <= KD31Data[SamplePositionIdx-1].SOKUTEI_NM ){
            if( n== ScrollBarA->Position || n== 0) return ;
...
void __fastcall TForm1::SetMEMO(void)
{
	...
	for( i=0; i<30; i++ )
    {
		...
		if( KD31Data[i].SAMPLE_P == -1 )
		{
			...
			if ( Trim(KD31Data[i].MEMO) != "" ) {
				//pPnlP[i]->Hint = KD31Data[i].MEMO;
				pEdt1[i]->Hint = KD31Data[i].MEMO;
				pEdt2[i]->Hint = KD31Data[i].MEMO;
				pEdt3[i]->Hint = KD31Data[i].MEMO;
				pEdt4[i]->Hint = KD31Data[i].MEMO;
				pScrl[i]->Hint = KD31Data[i].MEMO;
				pPnlN[i]->Hint = KD31Data[i].MEMO;
